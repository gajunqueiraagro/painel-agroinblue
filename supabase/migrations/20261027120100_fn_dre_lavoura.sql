...
-- md5(pg_get_functiondef) esperado: 79cf54e49c04a694f3f7f2915e6fb45c (len 13722). Conferir contra pg_proc, nao transcrever.
-- RPC unica do DRE da lavoura: uma chamada devolve culturas, linhas (ordem Conab + subtotais gerenciais),
-- centros (direto/rateado por cultura), investimento por centro, equilibrio, a pagar, rateio administrativo.
-- Tudo arredondado por folha (cultura x bloco x centro); subtotais somados no banco. O front nunca calcula.
...
centros_json AS (
  SELECT jsonb_agg(jsonb_build_object('bloco', bloco, 'centro', centro, 'por_cultura', pc, 'total', jsonb_build_object('direto', td, 'rateado', tr, 'a_pagar', ta)) ORDER BY bloco, td + tr DESC) AS j
  FROM (
    SELECT bloco, centro,
