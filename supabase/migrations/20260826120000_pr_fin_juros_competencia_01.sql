-- =====================================================================
-- PR-FIN-JUROS-COMPETENCIA-01
-- Move a competencia da parcela de JUROS do financiamento da data do
-- contrato para a data de VENCIMENTO, no writer oficial
-- public.fn_reconciliar_parcela_financiamento.
--
-- Uma unica linha executavel muda. Nenhuma outra.
--
-- AS TRES NATUREZAS DO FINANCIAMENTO, e por que so uma se move:
--
--   1 CAPTACAO — a entrada do dinheiro. Competencia = data_contrato, e a
--     data em que o recurso chegou. A 02B mediu 68 de 68 lancamentos de
--     captacao vinculados confirmando competencia, pagamento e ano_mes
--     nessa data. Fato economico instantaneo: nao se difere. NAO MUDA.
--
--   2 AMORTIZACAO (principal) — devolucao do principal captado. Nao e
--     despesa: e reducao de passivo. O fato economico que a origina ja
--     foi reconhecido na captacao, entao sua competencia permanece
--     ancorada no contrato. NAO MUDA. E' a linha 181 deste arquivo, e o
--     diff desta migration tem de deixa-la intacta.
--
--   3 JUROS — o custo do dinheiro no tempo. Diferente das outras duas, o
--     fato economico NAO e' instantaneo: ele se acumula ate o vencimento
--     da parcela, e e' no mes do vencimento que a despesa compete. Manter
--     juros na data do contrato empilha em um unico mes o custo de todo o
--     prazo do contrato, e e' o que esta frente corrige. MUDA.
--
-- COALESCE, e nao troca seca: se data_vencimento for nula, a competencia
-- cai de volta em v_contrato.data_contrato. A coluna e' nullable (a 02B
-- ja registrava isso em E5) e nenhum lancamento pode ficar sem
-- competencia. Medido hoje no Proto: 0 de 327 juros vinculados estao sem
-- vencimento, entao o ramo de fallback nasce inerte — existe pela coluna
-- ser nullable, nao por haver caso conhecido.
--
-- MEDICAO DO UNIVERSO (Proto, leitura em 2026-08-26, juros vinculados
-- por financiamento_parcelas.lancamento_juros_id, nao cancelados):
--   327 lancamentos de juros, R$ 23.656.698,81
--   277 ja estao com competencia = vencimento
--    49 estao com competencia = data_contrato, R$ 10.132.969,50
--     2 nao coincidem com nenhuma das duas
--     0 sem data_vencimento
--   68 meses de contrato distintos contra 147 meses de vencimento
--      distintos — a medida do empilhamento que esta frente desfaz.
-- A maioria da base ja segue a semantica nova; esta migration alinha o
-- WRITER ao que a base majoritariamente ja e'.
--
-- ZERO DML. Esta migration nao reescreve nenhum dos 49 lancamentos
-- historicos: ela so muda o que sera escrito nas proximas reconciliacoes.
-- O backfill dos 49 e' frente separada, nao decidida aqui. O Q9 herdado
-- prova a ausencia de DML por contagem antes/depois.
--
-- Base verbatim: 20260821120000_pr_fin_datas_vencimento_02b.sql
-- Corpo derivado programaticamente do arquivo da 02B, nunca redigitado.
-- md5 de origem: 7655e41017b74e7f272938deb87e5d76 (conferido vivo no Proto)
--
-- NAO altera o casamento parcela<->lancamento. NAO altera o bloco de
-- limpeza de totalizadores legados. NAO altera ACL, assinatura, owner,
-- SECDEF nem search_path. NAO altera a competencia do principal.
--
...
-- As guardas E1-E10 e os pos-checks Q1-Q6, Q8 e Q9 vem VERBATIM da 02B.
-- Somente o Q7 foi reescrito, porque o da 02B deixaria de discriminar
-- exatamente a troca que esta frente faz — ver o comentario no proprio Q7.
-- =====================================================================
DO $WRAP$
DECLARE
  c_esperado constant text := 'd76186edc628f6e3f4a8a4bb7bd2940253eb96a3b8ae3705d634e3092985aeb7';
  v_payload  constant text := $PAYLOAD$DO $BODY$
DECLARE
  -- ---------------------------------------------------------------------
  -- PR-FIN-JUROS-COMPETENCIA-01 — corpo executavel
  -- Alteracao cirurgica de public.fn_reconciliar_parcela_financiamento.
  -- UM ponto, e apenas um:
  --   J  a competencia da parcela de JUROS passa a sair de
  --      COALESCE(v_parcela.data_vencimento, v_contrato.data_contrato)
  -- Captacao, amortizacao do principal, vencimento, casamento estrutural,
  -- bloco de limpeza de totalizadores legados, valores, classificacao,
  -- status, cancelamento e calculo permanecem byte-identicos.
  -- Zero DML. Zero backfill. Nenhuma linha historica e alterada aqui.
  -- ---------------------------------------------------------------------
  c_md5_origem constant text := '7655e41017b74e7f272938deb87e5d76';
  c_md5_final  constant text := '38695f61d0f47ecf7e8b488956dd6b56';
  ...
  -- Literais que discriminam os dois blocos de competencia. Ver Q7.
  c_lit_princ  constant text := '''data_competencia'', v_contrato.data_contrato,';
  c_lit_juros  constant text := '''data_competencia'', COALESCE(v_parcela.data_vencimento, v_contrato.data_contrato),';
  c_bloco_j    constant text := 'v_novo_j := jsonb_build_object(';
...
BEGIN
  ...
  v_novo_j := jsonb_build_object(
    ...
    'descricao', v_desc_j,
    'data_pagamento', v_data_pag,
    'data_competencia', COALESCE(v_parcela.data_vencimento, v_contrato.data_contrato),
    'data_vencimento', v_parcela.data_vencimento,
    'status_transacao', v_status,
    'sinal','-1',
...
BEGIN
  ...
  SELECT count(*) INTO v_n
   ...
   WHERE NOT t.tgisinternal AND n.nspname = 'public'
     ...
     AND t.tgenabled = 'O';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 E1: trigger 02E ausente ou desabilitado — ano_mes nao seria derivado';
  END IF;
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 E2: corpo do 02E divergente — a autoridade de ano_mes nao e a auditada';
  END IF;
  ...
  IF v_n <> 11 THEN
    RAISE EXCEPTION 'JUROS-01 E3: esperava as 11 policies nominais do PR-SEC-RLS-FINANCIAMENTOS-01, encontrei %', v_n;
  END IF;
  ...
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'JUROS-01 E4: existe policy permissiva nas tabelas de financiamento (%)', v_n;
  END IF;
  ...
  IF v_n <> 4 THEN
    RAISE EXCEPTION 'JUROS-01 E5: colunas de data de financeiro_lancamentos_v2 nao estao no shape esperado (%)', v_n;
  END IF;
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 E6: financiamentos.data_contrato ausente — competencia da captacao indefinida';
  END IF;
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 E7: assinatura/retorno/linguagem/SECDEF/search_path/owner divergentes';
  END IF;

  IF obj_description(
       (SELECT p.oid FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
         WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento'), 'pg_proc') IS NOT NULL THEN
    RAISE EXCEPTION 'JUROS-01 E8: funcao passou a ter COMMENT — estado nao auditado';
  END IF;
  ...
  IF v_acl <> c_md5_acl THEN
    RAISE EXCEPTION 'JUROS-01 E9: ACL da funcao divergente (md5=%). Nao altero ACL nesta frente.', v_acl;
  END IF;
  ...
  IF v_md5 = c_md5_final THEN
    RAISE NOTICE 'JUROS-01: estado final ja vigente. NO-OP verdadeiro — nenhum comando emitido.';
    RETURN;
  ELSIF v_md5 <> c_md5_origem THEN
    RAISE EXCEPTION 'JUROS-01 E10: corpo divergente (md5=%). Nao uso CREATE OR REPLACE cego.', v_md5;
  END IF;
  ...
  SELECT md5(p.prosrc) INTO v_md5
   ...
   WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento';
  IF v_md5 <> c_md5_final THEN
    RAISE EXCEPTION 'JUROS-01 Q1: corpo final divergente (md5=%)', v_md5;
  END IF;
  ...
  SELECT count(*) INTO v_n
   ...
   WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento'
     ...
     AND pg_get_userbyid(p.proowner) = 'postgres';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q2: atributos da funcao mudaram apos o replace';
  END IF;
  ...
  SELECT md5(coalesce((SELECT string_agg(coalesce(pg_get_userbyid(a.grantee),'PUBLIC')||':'||a.privilege_type,'|'
   ...
   WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento';
  IF v_acl <> c_md5_acl THEN
    RAISE EXCEPTION 'JUROS-01 Q3: ACL da funcao mudou (md5=%)', v_acl;
  END IF;
  ...
  SELECT count(*) INTO v_n
   ...
   WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento';
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q4: existem % versoes da funcao', v_n;
  END IF;
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q5: bloco de limpeza de totalizadores legados foi alterado';
  END IF;
  ...
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q6: casamento estrutural (FK + marcador de observacao) foi alterado';
  END IF;

  -- Q7 competencia: PRINCIPAL na origem, JUROS no vencimento
  --    A checagem da 02B era  prosrc LIKE '%''data_competencia'', v_contrato.data_contrato%'
  --    e contava FUNCOES, nao ocorrencias. Depois desta frente ela deixaria de
  --    discriminar: o literal do principal continua no corpo, entao ela passaria
  --    mesmo que as duas linhas tivessem sido trocadas entre si. Aqui se conta
  --    cada literal e se prova a ORDEM relativa ao cabecalho do bloco de juros.
  --    Sem regex: strpos e contagem por diferenca de comprimento.
  SELECT p.prosrc INTO v_src
    FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
   WHERE n.nspname = 'public' AND p.proname = 'fn_reconciliar_parcela_financiamento';

  v_n := (length(v_src) - length(replace(v_src, c_lit_princ, ''))) / length(c_lit_princ);
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q7a: esperava 1 ocorrencia da competencia do PRINCIPAL, achei %', v_n;
  END IF;

  v_n := (length(v_src) - length(replace(v_src, c_lit_juros, ''))) / length(c_lit_juros);
  IF v_n <> 1 THEN
    RAISE EXCEPTION 'JUROS-01 Q7b: esperava 1 ocorrencia da competencia dos JUROS, achei %', v_n;
  END IF;

  IF NOT (strpos(v_src, c_lit_princ) < strpos(v_src, c_bloco_j)
      AND strpos(v_src, c_bloco_j)   < strpos(v_src, c_lit_juros)) THEN
    RAISE EXCEPTION 'JUROS-01 Q7c: literais de competencia fora dos blocos esperados (princ=%, bloco_j=%, juros=%)',
      strpos(v_src, c_lit_princ), strpos(v_src, c_bloco_j), strpos(v_src, c_lit_juros);
  END IF;
  ...
  IF v_n <> 0 THEN
    RAISE EXCEPTION 'JUROS-01 Q8: sobrou escrita executavel de ano_mes na funcao (% linhas)', v_n;
  END IF;
  ...
  IF v_fp_pos <> v_fp_antes THEN
    RAISE EXCEPTION 'JUROS-01 Q9: contagens mudaram (% -> %)', v_fp_antes, v_fp_pos;
  END IF;

  RAISE NOTICE 'JUROS-01: aplicado. Competencia dos JUROS passa ao vencimento; principal e captacao intocados. Fingerprint: %', v_fp_pos;
END
$BODY$;
$PAYLOAD$;
  v_hash text;
BEGIN
  ...
  IF v_hash <> c_esperado THEN
    RAISE EXCEPTION 'JUROS-01: payload adulterado. sha256 esperado=% obtido=%. Nada foi executado.',
      c_esperado, v_hash;
  END IF;
  EXECUTE v_payload;
END
