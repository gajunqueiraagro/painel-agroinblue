-- 20260630_pr_p32_candidato_ocupado.sql
-- PR-P3.2 — Estação (modo sistema_sem_vinculo): marcar/rebaixar candidato OFX já vinculado.
--
-- Causa: fn_ws_conciliacao, ao gerar candidatos de extrato ('ofx_para_sistema'), não
-- informava se o extrato já tem CBI ativo. Oferecia extrato OCUPADO como candidato →
-- operador clica Vincular → guard do banco RAISE "extrato ja possui vinculo ativo".
--
-- Correção (NÃO esconde; marca+rebaixa+explica). 2 patches no bloco ofx_para_sistema:
--   1.1  candidato ganha extrato_ja_vinculado (EXISTS CBI ativo) + lancamento_vinculado_id.
--        Fonte de verdade = CBI ativo (desfeito_em IS NULL); NÃO usa e.status (pode ficar stale).
--   1.2  ORDER BY rebaixa ocupados (extrato_ja_vinculado ASC NULLS FIRST = livres primeiro),
--        preservando os critérios atuais. Rebaixar ≠ esconder; WHERE/LIMIT intactos.
-- NÃO toca o bloco sistema_para_ofx, v_vincular, candidatos_financeiros, filtros nem confiança.
-- Corpo materializado do banco VIVO + os 2 str_replace (md5 conferido). Forward-only.
...
REVOKE EXECUTE ON FUNCTION public.fn_ws_conciliacao(text,uuid) FROM PUBLIC, anon;
GRANT  EXECUTE ON FUNCTION public.fn_ws_conciliacao(text,uuid) TO authenticated;
