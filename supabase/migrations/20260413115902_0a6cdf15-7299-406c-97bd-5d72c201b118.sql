...
-- Recreate vw_zoot_categoria_mensal with cross-year LAG window
...
WITH categorias AS (
    ...
    SELECT l.fazenda_id, l.cliente_id, cr.id AS categoria_id,
           EXTRACT(year FROM l.data::date)::integer AS ano, EXTRACT(month FROM l.data::date)::integer AS mes,
           sum(l.quantidade) AS qtd, sum(l.quantidade::numeric * COALESCE(l.peso_medio_kg, 0::numeric)) AS peso
    FROM lancamentos l JOIN categorias cr ON cr.codigo = l.categoria
    ...
    SELECT mg.fazenda_id, cr.id AS categoria_id,
           ...
           mg.gmd_previsto
    FROM meta_gmd_mensal mg JOIN categorias cr ON cr.codigo = mg.categoria
), mov_all AS (
    ...
    FROM mov_cat_bases mc JOIN categorias cr ON cr.id = mc.categoria_id
    ...
    SELECT expanded.*,
           ...
           expanded.peso_ini + COALESCE(sum(expanded.peso_entradas_ext - expanded.peso_saidas_ext + expanded.peso_evol_entrada - expanded.peso_evol_saida) OVER w_prev, 0::numeric) AS peso_contabil_inicio_mes
    FROM expanded
    ...
    SELECT DISTINCT ON (fp.pasto_id, fp.ano_mes) fp.id, fp.fazenda_id, fp.ano_mes, fp.pasto_id
    ...
    FROM fechamento_dedup fd JOIN fechamento_pasto_itens fpi ON fpi.fechamento_id = fd.id
    ...
    SELECT r.fazenda_id, r.cliente_id, r.ano, r.mes, r.cenario,
           ...
           CASE
               ...
               ELSE round(r.peso_ini + COALESCE(sum(r.peso_entradas_ext - r.peso_saidas_ext + r.peso_evol_entrada - r.peso_evol_saida) OVER (PARTITION BY r.fazenda_id, r.ano, r.cenario, r.categoria_id ORDER BY r.mes ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 0::numeric)
                    + COALESCE(sum(COALESCE(mg.gmd_previsto, 0::numeric) * ((r.cab_inicio_mes + r.cab_final) / 2.0) * EXTRACT(day FROM make_date(r.ano, r.mes, 1) + '1 mon'::interval - '1 day'::interval)) OVER (PARTITION BY r.fazenda_id, r.ano, r.cenario, r.categoria_id ORDER BY r.mes ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 0::numeric), 2)
           END AS ptf,
           CASE
    ...
    SELECT pre_final.*,
           -- KEY FIX: partition WITHOUT ano so LAG crosses year boundaries (Dec Y-1 -> Jan Y)
           lag(pre_final.sf) OVER w_cat AS prev_sf,
           lag(pre_final.ptf) OVER w_cat AS prev_ptf,
           lag(pre_final.fonte) OVER w_cat AS prev_fonte
    FROM pre_final
    WINDOW w_cat AS (PARTITION BY pre_final.fazenda_id, pre_final.cenario, pre_final.categoria_id ORDER BY pre_final.ano, pre_final.mes)
)
SELECT fazenda_id, cliente_id, ano, mes, cenario,
       ...
       -- saldo_inicial: prefer previous month closing (cross-year), fallback to saldos_iniciais for first-ever month
       CASE
           WHEN prev_sf IS NOT NULL THEN
               CASE WHEN prev_fonte = 'fechamento' THEN prev_sf ELSE cab_inicio_mes::integer END
           ELSE cab_ini::integer  -- first month with data (no previous month exists)
       ...
       sf AS saldo_final,
       -- peso_total_inicial: same cross-year logic
       CASE
           WHEN prev_ptf IS NOT NULL THEN
               CASE WHEN prev_fonte = 'fechamento' THEN prev_ptf
                    WHEN cenario = 'meta' THEN COALESCE(prev_ptf, round(peso_contabil_inicio_mes, 2))
                    ELSE round(peso_contabil_inicio_mes, 2)
               END
           ELSE round(peso_ini, 2)  -- first month
       END AS peso_total_inicial,
       ptf AS peso_total_final,
       CASE
           WHEN (CASE WHEN prev_sf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_sf ELSE cab_inicio_mes::integer END ELSE cab_ini::integer END) > 0
           THEN round(
               (CASE WHEN prev_ptf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_ptf WHEN cenario = 'meta' THEN COALESCE(prev_ptf, round(peso_contabil_inicio_mes, 2)) ELSE round(peso_contabil_inicio_mes, 2) END ELSE round(peso_ini, 2) END)
               /
               (CASE WHEN prev_sf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_sf::numeric ELSE cab_inicio_mes END ELSE cab_ini::numeric END)
           , 2)
           ELSE NULL::numeric
       ...
       -- GMD
       CASE
           WHEN ((CASE WHEN prev_sf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_sf::numeric ELSE cab_inicio_mes END ELSE cab_ini::numeric END + sf::numeric) / 2.0) > 0
                AND EXTRACT(day FROM make_date(ano, mes, 1) + '1 mon'::interval - '1 day'::interval) > 0
           THEN round(
               (ptf
                - (CASE WHEN prev_ptf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_ptf WHEN cenario = 'meta' THEN COALESCE(prev_ptf, round(peso_contabil_inicio_mes, 2)) ELSE round(peso_contabil_inicio_mes, 2) END ELSE round(peso_ini, 2) END)
               ...
               / ((CASE WHEN prev_sf IS NOT NULL THEN CASE WHEN prev_fonte = 'fechamento' THEN prev_sf::numeric ELSE cab_inicio_mes END ELSE cab_ini::numeric END + sf::numeric) / 2.0)
               / EXTRACT(day FROM make_date(ano, mes, 1) + '1 mon'::interval - '1 day'::interval)
           , 4)
           ELSE NULL::numeric
       END AS gmd,
       -- producao_biologica
