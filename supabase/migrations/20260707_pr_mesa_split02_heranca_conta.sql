-- ============================================================================
-- PR-MESA-SPLIT-02 — Herança de conta com fallback ao extrato no split.
--
-- "Detalhe sem conta é lançamento invisível — nunca criar." [Princípio 9]
-- Defeito: o consolidado (ex.: 3fea113f) pode ter conta_bancaria_id = NULL; o split
-- herdava a conta SÓ do consolidado → os N detalhes nasceriam sem conta (invisíveis na
-- régua por conta e no matcher futuro). O extrato religado é a verdade bancária → fallback.
--
-- CREATE OR REPLACE do fn_classificacao_split_substituir (corpo VERBATIM do SPLIT-01
-- (+fix), já aplicado) + 2 deltas. Mesma assinatura (uuid, uuid, uuid[]) → sem overload.
--
-- DELTA 1 — herança de conta com fallback ao extrato:
--   conta_bancaria_id := COALESCE(v_lan.conta_bancaria_id, v_ext.conta_bancaria_id);
--   conta_destino_id  := v_lan.conta_destino_id (INALTERADO). ÂNCORA CONFIRMADA:
--   extrato_bancario_v2 tem conta_bancaria_id (NOT NULL) mas NÃO tem conta_destino_id
--   (OFX é de uma conta só) → não há fallback de destino; não inventamos coluna.
-- DELTA 2 — guard (i) após (h): se a conta_bancaria_id herdada for NULL →
--   'conta_indeterminada' (defensivo; extrato.conta_bancaria_id é NOT NULL, então na
--   prática não dispara — declarado no relatório).
-- Nada mais muda (a→h, rastreabilidade, staging, cancelamento, religamento, cbi: verbatim).
-- ============================================================================
...
DECLARE
  ...
  v_conta_bancaria uuid; v_conta_destino uuid;   -- PR-MESA-SPLIT-02: conta herdada (fallback extrato)
BEGIN
  ...
  -- PR-MESA-SPLIT-02 · DELTA 1 — conta herdada com fallback ao extrato (mesma p/ os N).
  -- extrato_bancario_v2 NÃO tem conta_destino_id (OFX é de uma conta só) → destino fica o
  -- do consolidado (inalterado); só conta_bancaria_id ganha fallback ao extrato.
  v_conta_bancaria := COALESCE(v_lan.conta_bancaria_id, v_ext.conta_bancaria_id);
  v_conta_destino  := v_lan.conta_destino_id;

  -- PR-MESA-SPLIT-02 · DELTA 2 — GUARD (i): sem conta bancária determinável não se cria
  -- detalhe invisível (avaliado UMA vez — a conta é a mesma para os N detalhes).
  IF v_conta_bancaria IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'motivo', 'conta_indeterminada',
      'mensagem', 'Não foi possível determinar a conta bancária do lançamento. Corrija o consolidado ou o extrato antes de substituir.');
  END IF;
  ...
  LOOP
    INSERT INTO financeiro_lancamentos_v2 (
      ...
      v_conta_bancaria, v_conta_destino,
