-- PR-OC-VENDA-ACABAMENTO (B-08 item 1) — o guard nomeia o campo QUE EXISTE NA TELA.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. Registro 20260831122848; o nome do arquivo usa
-- 20260831123000 por instrucao do briefing.
--
-- O DEFEITO, medido em B-05: esta funcao recusava um realizado incompleto dizendo
-- "Realizado do boitel incompleto. Falta preco de venda da @" — e o modal do REALIZADO nao
-- tem campo de preco da @. La' se digita o VALOR TOTAL DO ABATE e as ARROBAS do papel, e o
-- preco e' que deriva (`valorTotalAbate / aTS`). O operador era mandado procurar um campo
-- inexistente, e a mensagem nao dizia o que de fato faltava.
--
-- ⚠ E ERA A UNICA SUPERFICIE FORA DO ALCANCE DO FRONT. A correcao equivalente no lado da
-- tela (`exigencias(d, cenario)`) saiu em B-05; esta frase, porem, nasce no SQL e chega
-- por toast, entao so' uma migration a alcanca. Foi o que aquele relatorio registrou como
-- residuo, e e' o que este arquivo fecha.
--
-- ⚠ QUATRO CASES, E NAO CINCO. `dias` fica como esta': ele e' campo digitavel nos DOIS
-- cenarios, com o mesmo nome. Os outros quatro invertem de direcao no realizado — o
-- driver unitario da projecao vira consequencia de um total do papel —, e e' por isso que
-- cada um nomeia o TOTAL que o operador realmente digita.
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO: corpo de 20260831102614 (md5 f4ad99e4...,
-- 15935 caracteres, reconferido nesta sessao) com os quatro `array_append` trocados por
-- `CASE v_row.cenario`. Nada mais foi tocado — nem a lista branca, nem as 41 colunas, nem
-- a autorizacao. Conferido pelo oraculo — md5 6fbb63f12fa93147868036c9ecb42c5b, 16279
-- caracteres, byte a byte igual ao aplicado.
