-- 20260909091649 — ano_mes volta a ser derivado pelo trigger
-- A migration anterior tambem passou a mandar ano_mes = to_char(v_ext.data_movimento, ...).
-- Era inerte: trg_00_ano_mes_from_competencia deriva ano_mes de data_competencia em INSERT
-- e UPDATE e descarta o que vier no payload — foi por isso que o UPDATE de backfill da
-- migration anterior nao teve efeito. A expressao original volta, agora com o comentario
-- dizendo que e' inerte, para a proxima leitura nao tentar de novo.
-- O conserto de 091205 (data_pagamento = v_ext.data_movimento) PERMANECE.
-- Aplicada no proto pela Management API em 09/09; versionada aqui depois, conferida por md5.
...
BEGIN
  ...
  LOOP
    INSERT INTO financeiro_lancamentos_v2 (
      ...
      COALESCE(v_s.excel_ano_mes, to_char(v_s.excel_data, 'YYYY-MM')), -- inerte: trg_00_ano_mes_from_competencia deriva de data_competencia
      v_s.excel_data, v_ext.data_movimento, v_s.excel_valor, -- data_pagamento = data do extrato vinculado
 v_lan.sinal, v_lan.tipo_operacao,
      'realizado',
