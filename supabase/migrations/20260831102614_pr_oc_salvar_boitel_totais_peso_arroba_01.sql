-- PR-OC-VENDA-REALIZADO-02G — a lista branca aceita os dois totais da balanca.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. Registro 20260831102614.
--
-- QUINTA vez na mesma frente, e a razao e' sempre a mesma: esta RPC ENUMERA as colunas.
-- Coluna nova sem entrada aqui e' descartada EM SILENCIO — e a auditoria, que grava o
-- payload inteiro, ainda diz que gravou. Coluna e lista branca andam SEMPRE em par.
--
-- ⚠ SEM `COALESCE`, como os quatro anteriores: as colunas sao nullable e sem default, e
-- nulo e' valor legitimo ("o papel ainda nao chegou"). Forcar zero transformaria ausencia
-- em fato — "pesou zero" — que e' o oposto do que a coluna diz.
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO: corpo de 20260831100705 (md5 b8ad6ab1...,
-- 15382 caracteres, reconferido nesta sessao) com as TRES insercoes nos mesmos tres
-- pontos de sempre, apos `valor_total_diarias`. Conferido pelo oraculo — md5
-- f4ad99e40953cf8a4fba0867b22cbd60, 15935 caracteres, byte a byte igual ao aplicado. As
-- demais 39 colunas nao foram tocadas.
-- ⚠ O ALINHAMENTO E' PARTE DO BYTE. Na primeira tentativa as duas linhas novas do
-- ON CONFLICT SET sairam com TRES espacos a mais cada uma (15941 x 15935) — o oraculo
-- pegou. As colunas `=`, `THEN` e `ELSE` do bloco tem posicao fixa, e a linha nova se
-- alinha a ELAS, nao a uma largura recontada do nome da coluna.
