-- PR-OC-VENDA-CASCATA-BOLSO-01 (adendo A) — a lista branca aceita `outros_no_boitel`.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. Registro 20260830184124.
--
-- Mesma razao da irma 20260830182000: esta RPC nao escreve a tabela por reflexao — ela
-- ENUMERA as colunas, e uma coluna nova sem entrada aqui seria descartada em silencio,
-- com a auditoria (que grava o payload inteiro) ainda dizendo que gravou. E' a segunda
-- barreira da familia, e ja custou uma PARADA nesta frente.
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO: corpo de 20260830182000 (md5 268525ab...,
-- identico ao entao aplicado) com as TRES insercoes de `outros_no_boitel` nos mesmos tres
-- pontos das cinco anteriores. Conferido pelo oraculo — md5 08b3015ea835b0d17cd481a31296826b,
-- 14345 caracteres, byte a byte igual ao aplicado. As demais 34 colunas nao foram tocadas.
...
BEGIN
  ...
  INSERT INTO public.zoo_operacao_boitel AS b (
    ...
    custo_frete_no_boitel, despesas_abate_no_boitel, custo_notas_envio, notas_envio_no_boitel, data_abate, outros_no_boitel,
