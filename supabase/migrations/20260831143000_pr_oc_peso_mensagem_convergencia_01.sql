-- PR-OC-PESO-MENSAGEM-CONVERGENCIA-01 (B-13) — as duas verdades voltam a ser uma.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. Registro 20260831142736.
--
-- FECHA A HISTORIA DA MIGRATION INFIEL. Em 20260831120000 o arquivo versionado registrou
-- uma guarda de peso obrigatorio com UMA frase; o banco recebeu OUTRA. Nao foi formatacao:
-- era o texto que o operador ve e o fallback da categoria.
--     versionado : 'Informe o peso medio do lote % (ordem %). Lote sem peso nao pode ser salvo.',
--                  COALESCE(NULLIF(v_lote->>'categoria_negociada',''), 'sem categoria')
--     aplicado   : 'Peso medio obrigatorio no lote % (ordem %)',
--                  COALESCE(v_lote->>'categoria_negociada','?')
-- O repositorio guardou a INTENCAO e o banco guardou outra coisa; por seis dias as duas
-- conviveram sem que nada acusasse, porque nenhum gate compara o corpo de uma funcao com o
-- arquivo que diz te-la criado.
--
-- ⚠ QUEM PEGOU FOI O ORACULO, e nao um gate: em B-12, ao reconstruir a base de
-- `oc_salvar_lotes` para versionar outra mudanca, o md5 do repo (c3c7e65a, 8665) nao bateu
-- com o do aplicado (8d5d39ef, 8594). A divergencia foi isolada AO BYTE — trocando so'
-- aquela guarda, os dois convergiam. LICAO: o oraculo de md5 nao serve so' para provar o
-- que se acabou de escrever; ele AUDITA O HISTORICO. Registro infiel fica invisivel ate'
-- alguem tentar reconstruir a partir dele.
--
-- ⚠ A DECISAO FOI DO GABRIEL, e foi pela frase do REPOSITORIO: ela INSTRUI ("Informe o
-- peso medio...") em vez de so' constatar ("Peso medio obrigatorio"), e o fallback com
-- `NULLIF` imprime "sem categoria" onde o outro imprimia string vazia — categoria em
-- branco e' ausencia, e ausencia tem nome.
-- ⚠ MIGRATION JA APLICADA NAO SE EDITA: a 120000 fica como esta', com o seu registro
-- historico. A convergencia acontece PARA A FRENTE, aqui, onde ela e' rastreavel.
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO: corpo de 20260831140238 (md5 568a6300...,
-- 9281 caracteres, reconferido nesta sessao) com os DOIS literais da guarda trocados.
-- Nada mais foi tocado — nem o CASE que protege o realizado, nem o carimbo de
-- `updated_at`, nem as demais 150 linhas. Conferido pelo oraculo — md5
-- cb35bd9d095d6e90a90e14e91418c98e, 9338 caracteres, byte a byte igual ao aplicado.
...
BEGIN
  ...
  LOOP
    ...
    IF v_peso IS NULL OR v_peso <= 0 THEN RAISE EXCEPTION 'Informe o peso medio do lote % (ordem %). Lote sem peso nao pode ser salvo.', COALESCE(NULLIF(v_lote->>'categoria_negociada',''), 'sem categoria'), v_ordem USING ERRCODE = 'P0001'; END IF;
