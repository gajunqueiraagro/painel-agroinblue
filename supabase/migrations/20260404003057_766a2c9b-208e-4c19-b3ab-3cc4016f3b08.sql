...
SELECT r.fazenda_id, r.cliente_id, r.ano, r.mes, r.cenario,
    ...
    -- Peso médio final: fechamento vence (só para realizado)
    CASE
        WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0
            THEN round(pr.peso_total_real / pr.cab_pesado, 2)
        WHEN r.cab_final > 0
            THEN round(r.peso_final_contabil / r.cab_final, 2)
        ELSE NULL
    ...
    -- GMD: usa peso final do fechamento quando disponível (só realizado)
    CASE
    ...
    -- UA média: usa peso médio do fechamento quando disponível
    CASE
        WHEN ((COALESCE(r.cab_inicio_mes, 0) + COALESCE(
            CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END
        , 0)) / 2.0) > 0
    ...
    -- Campo de rastreabilidade: fonte oficial do mês
    CASE
