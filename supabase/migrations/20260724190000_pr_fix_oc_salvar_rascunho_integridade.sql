-- Restaura a integridade da oc_salvar_rascunho após a redefinição multilote:
-- preserva a lógica multilote da 01b e reintegra numero_documento, campos
-- estruturados de abate e guards de aplicabilidade perdidos na sobrescrita.
-- Futuras redefinições desta RPC devem ser comparadas integralmente com o
-- corpo vigente para evitar perda silenciosa de campos e validações.
--
-- Base: corpo VIGENTE da 20260722220000 (RECEB-01B multilote) — v_completo
--   single-lot OU multilote mantido byte a byte. Reintegração VERBATIM da
--   20260720180100 (02b_2, ADR-2026-16 D3.B): bloco v_tipo_efetivo + guards de
--   abate; numero_documento + 7 campos de abate no INSERT e no CASE do UPDATE
--   (preservam valor atual quando a chave é omitida). cenario permanece FORA do
--   UPDATE (contrato vigente inalterado). Assinatura inalterada => CREATE OR
--   REPLACE sem DROP. Nenhuma regra nova inventada.
