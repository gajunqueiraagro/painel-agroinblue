-- PR-FIN-DATAS-04B — preservar o VENCIMENTO no título da Operação Comercial.
--   CREATE OR REPLACE de public.oc_gerar_obrigacoes copiado VERBATIM da definição vigente
--   (âncora: 20260726170000_pr_oc_fin_contrato_obrigacao_01_fix_uuid.sql, linhas 13-220),
--   preservando integralmente: assinatura, parâmetros, RETURNS, LANGUAGE, SECURITY DEFINER,
--   search_path, REVOKE/GRANT, ACL/tenant guard, validações, guarda de 2ª base, classificação,
--   idempotência, criação de partes/eventos, vínculos, status, origem, ano_mes, data_competencia
--   e a expressão legada de data_pagamento (COALESCE de três níveis com data_operacao).
--   ÚNICA MUDANÇA (aditiva): o INSERT em financeiro_lancamentos_v2 passa a gravar a coluna
--     data_vencimento := COALESCE(v_venc, v_op.data_pagamento_prevista)
--   — vencimento próprio da obrigação (zoo_operacao_partes.data_vencimento) com fallback ao
--   vencimento previsto da operação. NUNCA data_operacao, NUNCA derivado de data_pagamento.
--   Create-only: sem UPDATE de títulos existentes, sem reprojeção, sem alterar IDs.
--   NÃO toca oc_sincronizar, writers legados, tabelas, colunas, índices, triggers ou dados.
--   Aplicar SOMENTE no Proto (binbcdfbisgscrifztia).
  ...
  LOOP
    ...
    IF v_materializar THEN
      ...
      INSERT INTO public.financeiro_lancamentos_v2 (
        ...
        v_op.data_operacao, v_data_pag, COALESCE(v_venc, v_op.data_pagamento_prevista), to_char(v_op.data_operacao,'YYYY-MM'),
