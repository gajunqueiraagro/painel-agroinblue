-- PR-CONC-GRUPO-FASE-1 — Migration 06 (CORRETIVA): fn_vincular_grupo_conciliacao sem audit explícito.
-- A auditoria é trigger-driven (trg_audit_conciliacao): cada INSERT de membro gera 'conciliacao_criada'
-- com payload_depois=to_jsonb(NEW) contendo grupo_id e tipo_aprovacao. Removido o INSERT explícito em
-- conciliacao_audit_log (era redundante e usava acao fora do CHECK). Validações e retorno inalterados.
...
BEGIN
  ...
  -- Auditoria: trigger-driven (trg_audit_conciliacao grava 'conciliacao_criada' por INSERT acima).

  RETURN jsonb_build_object(
    'ok', true, 'grupo_id', v_grupo_id, 'extrato_id', p_extrato_id,
    'total_ofx', abs(v_ext.valor), 'total_aplicado', v_total,
    'diferenca', abs(v_ext.valor) - v_total, 'status_extrato', v_status,
    'tipo_aprovacao', 'agrupamento_manual', 'itens', v_itens
  );
END;
$function$;
...
-- ROLLBACK: reaplicar a versão da Migration 02 (com audit explícito) — não recomendado (viola acao CHECK).
