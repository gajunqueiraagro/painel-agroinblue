...
BEGIN
  ...
  LOOP
    ...
    -- PR6.2.1 — LV2 não tem coluna produto: preserva o produto dentro da descricao.
    INSERT INTO financeiro_lancamentos_v2 (
      ...
      CASE WHEN NULLIF(btrim(v_row.produto), '') IS NOT NULL AND NULLIF(btrim(v_row.descricao), '') IS NOT NULL THEN v_row.produto || ' — ' || v_row.descricao WHEN NULLIF(btrim(v_row.produto), '') IS NOT NULL THEN v_row.produto ELSE NULLIF(btrim(v_row.descricao), '') END, v_row.observacao, v_fornecedor_id,
