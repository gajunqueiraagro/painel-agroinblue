-- PR-DRE-PECUARIA-01 — `Tributos` entra na lista de macros que NAO sao custo administrativo.
--
-- ⚠ UMA TROCA EM CADA FUNCAO, TODAS AS OCORRENCIAS: `'Saida Financeira','Transferencias')` ganha
--   `,'Tributos')`. Uma ocorrencia em `fn_dre_lavoura`, quatro em `fn_painel_rateio_detalhe` —
--   e as quatro precisam mudar juntas, senao o pool do modal discorda do pool da grade.
-- ⚠ O EFEITO E' NO NUMERO DA LAVOURA: o rateio administrativo do amendoim 25/26 cai de
--   189.375,09 para 167.536,01. Tributo pago pelo escritorio nao e' custo de operar a fazenda;
--   estava inflando o pool que se reparte por area.
-- ⚠ JA APLICADA NO PROTO (16/09, GO do Gabriel). Delta conferido por md5 nas duas pontas —
--   reconstrucao e `md5(prosrc)` do banco: fn_dre_lavoura e7d7ab74161428f7509b3f167b561034
--   (13.576) e fn_painel_rateio_detalhe 14efb5024a68f7f95a336024c1856f4f (5.204).
-- ⚠ A ACL SO' ACOMPANHA A SECURITY DEFINER: `fn_dre_lavoura` e' INVOKER (`security definer` =
--   false, medido) e nao leva revoke/grant; a outra e' definer e leva.
...
begin
  ...
  if p_tipo in ('natureza','investimento','pool_fixo','pool_investimento') then
    ...
    select coalesce(jsonb_agg(jsonb_build_object('id',l.id,'data',l.data_competencia,'descricao',l.descricao,'favorecido',coalesce(f.nome_favorecido,f.nome,'-'),'valor',l.valor,'compartilhado',true) order by l.data_competencia),'[]'::jsonb)
      ...
      where l.cliente_id=p_cliente and coalesce(l.cancelado,false)=false and l.escopo_negocio='administrativo' and l.tipo_operacao='2-Saídas' and l.macro_custo not in ('Dividendos','Investimento na Fazenda','Saída Financeira','Transferências','Tributos') and l.data_competencia between v_ini and v_fim;
  end if;
