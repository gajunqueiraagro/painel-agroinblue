-- 20260909091205 — split grava a data do extrato
-- Os filhos do split nasciam com data_pagamento = data da PLANILHA (v_s.excel_data). A
-- funcao ja carregava o extrato vinculado em v_ext (extrato_bancario_v2%ROWTYPE) para
-- validar o valor, mas nao usava a data dele: o filho ficava com data diferente do extrato
-- conciliado e o mes nao fechava — caso Agnaldo, 13 linhas alinhadas a mao em 08/09.
...
BEGIN
  ...
  LOOP
    INSERT INTO financeiro_lancamentos_v2 (
      ...
      to_char(v_ext.data_movimento, 'YYYY-MM'),
      v_s.excel_data, v_ext.data_movimento, v_s.excel_valor, v_lan.sinal, v_lan.tipo_operacao,
      'realizado',
...
-- sem efeito: trg_00_ano_mes_from_competencia restaura OLD.ano_mes
UPDATE financeiro_lancamentos_v2
   SET ano_mes = to_char(data_pagamento,'YYYY-MM')
 WHERE origem_lancamento = 'mesa_split'
   AND ano_mes <> to_char(data_pagamento,'YYYY-MM');
