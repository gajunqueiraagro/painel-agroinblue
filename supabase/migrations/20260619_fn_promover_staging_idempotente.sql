-- 20260619_fn_promover_staging_idempotente.sql
-- STAGING-02 — 1 OFX -> 1 lancamento estrito + promocao idempotente.
-- Skip de pendente cujo OFX ja tem irmao promovido (qualquer sessao / mesmo loop).
-- Orfaos (ofx_extrato_id NULL) nao sao afetados (fora do indice parcial).
-- Guard "sem pendentes" deixa de lancar excecao -> retorno amigavel (idempotencia).
...
BEGIN
  ...
  IF v_count_pendente = 0 THEN
    RETURN jsonb_build_object('ok', true, 'sessao_id', p_sessao_id,
      'promovidos', 0, 'ja_promovidos', 0, 'motivo', 'sem_pendentes');
  END IF;
  ...
  LOOP
    -- STAGING-02 — 1 OFX -> 1 lancamento estrito + idempotencia.
    -- Pula pendente cujo OFX ja tem irmao promovido (qualquer sessao, ou
    -- promovido mais cedo neste mesmo loop). Orfaos (ofx NULL) seguem normais.
  ...
  RETURN jsonb_build_object('ok', true, 'sessao_id', p_sessao_id,
    'promovidos', v_promovidos, 'ja_promovidos', v_ja_promovido_ofx);
END;
$function$;
