-- 20260907112944 extrato_conciliar_mes_lote (aplicada no proto via Management API em 07/09/2026)
DROP FUNCTION IF EXISTS public.fn_extrato_conciliar_mes(uuid,uuid,text,boolean);
CREATE OR REPLACE FUNCTION public.fn_extrato_conciliar_mes(p_cliente_id uuid, p_conta_bancaria_id uuid, p_ano_mes text, p_simular boolean DEFAULT true, p_limite integer DEFAULT NULL::integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'movimentos_extrato', v_ja + jsonb_array_length(v_crus) + jsonb_array_length(v_subs) + v_restantes,
    'ja_conciliados', v_ja,
...
END;
$function$;
REVOKE ALL ON FUNCTION public.fn_extrato_conciliar_mes(uuid,uuid,text,boolean,integer) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.fn_extrato_conciliar_mes(uuid,uuid,text,boolean,integer) TO authenticated;
