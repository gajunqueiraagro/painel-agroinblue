-- LEGADO-ADM-03 (22/09/2026) - legado da fazenda Administrativo do Santa Rita Agro sai do custo de pecuaria
--
-- Migration de DADO, so do cliente Santa Rita Agro (77d37bbf). Nenhuma RPC, nenhuma tela.
...
--
-- Universo (medido em 22/09/2026 depois da ultima edicao manual, 10:55:25, com as edicoes congeladas): 766 lancamentos,
-- R$ 3061218.25, de plano pecuaria/agricultura na fazenda Administrativo, fora juros,
-- amortizacao e captacao (que a regra de 22/09 permite no administrativo da atividade).
--
-- REGRAS (a primeira que casa vence):
--   A_consorcio   o = 9030 and descricao ilike '%cons_rcio%'                             -> plano 22030, ADM
...
--   D_fazenda     o in (6100, 6110, 6120, 6130, 6140, 6150, 6160, 6170, 6240, 6250, 6260 -> plano mantido, SR
--   E: recorrencias 9f2ece7a -> 21110, c105b4a0 -> 22030 (texto do subcentro; safra zerada)
--
-- Carimbo em `observacao` (append ' | '): "Reclassificado em 22/09/2026 (LEGADO-ADM-03): era <subcentro
...
do $lote$
declare
  c   constant uuid := '77d37bbf-a440-4fca-bf1a-eac60cf91bc4';
  adm constant uuid := 'cfe61cf4-5c33-44f4-9e75-7a0fcc634799';
  v_regras constant text[] := array['A_consorcio', 'B_tributos'];
  n int; v_col int; v_s numeric;
begin
  -- -- ENTRADA ----------------------------------------------------------------
  create temp table _lg_faz on commit drop as select * from (values
      ('ADM', 'cfe61cf4-5c33-44f4-9e75-7a0fcc634799'::uuid),
      ('SR', '161b905e-f14c-4a9b-965f-dd3c8f82dc74'::uuid)) v(cod, id);
  ...
  create temp table _lg_plano on commit drop as
    select p.ordem_exibicao o, p.id, p.escopo_negocio esc, p.subcentro sub from public.financeiro_plano_contas p
     where p.cliente_id is null and p.ativo and p.ordem_exibicao in (17150, 21020, 21040, 21060, 21080, 21090, 21100, 21110, 21120, 21130, 21140, 21150, 21160, 22030);
  select count(*) into n from (select o from _lg_plano group by o having count(*) = 1) z;
  if n <> 14 or (select count(*) from _lg_plano) <> 14 then raise exception 'L1: planos de destino: % de 14', n; end if;
  ...
  with u as (
    select l.id, l.valor, l.plano_conta_id plano_origem, coalesce(l.descricao, '') descricao,
     ...
     where l.cliente_id = c and l.fazenda_id = adm and p.escopo_negocio in ('pecuaria', 'agricultura')
       ...
       and coalesce(l.observacao, '') not like '%LEGADO-ADM-03%'
  ), cl as (
    select u.*, case
      when o = 9030 and descricao ilike '%cons_rcio%' then 'A_consorcio'
  ...
  create temp table _lg_rec on commit drop as select * from (values
      ('9f2ece7a-ecf5-437d-a8ad-d16a85136548'::uuid, 21110),
      ('c105b4a0-909f-4c38-9d9a-a2fcd507df22', 22030)) e(id, o);
  ...
  update public.financeiro_recorrencias r
     set subcentro = pd.sub, safra_id = null, updated_at = now(),
         observacao = concat_ws(' | ', nullif(btrim(r.observacao), ''),
           'Reclassificado em 22/09/2026 (LEGADO-ADM-03): era ' || r.subcentro || ' na fazenda Administrativo')
  ...
  update public.financeiro_lancamentos_v2 l
     set plano_conta_id = coalesce(t.plano_destino, l.plano_conta_id),
         observacao     = concat_ws(' | ', nullif(btrim(l.observacao), ''),
                            'Reclassificado em 22/09/2026 (LEGADO-ADM-03): era ' || t.sub || ' na fazenda Administrativo'
  ...
  select count(*) into n
   ...
   where l.plano_conta_id is distinct from coalesce(t.plano_destino, t.plano_origem)
      ...
      or coalesce(l.observacao, '') not like '%LEGADO-ADM-03%'
...
begin
  ...
  create temp table _lg_plano on commit drop as
    select p.ordem_exibicao o, p.id, p.escopo_negocio esc, p.subcentro sub from public.financeiro_plano_contas p
     where p.cliente_id is null and p.ativo and p.ordem_exibicao in (17150, 21020, 21040, 21060, 21080, 21090, 21100, 21110, 21120, 21130, 21140, 21150, 21160, 22030);
  select count(*) into n from (select o from _lg_plano group by o having count(*) = 1) z;
  if n <> 14 or (select count(*) from _lg_plano) <> 14 then raise exception 'L2: planos de destino: % de 14', n; end if;
...
begin
  ...
  create temp table _lg_plano on commit drop as
    select p.ordem_exibicao o, p.id, p.escopo_negocio esc, p.subcentro sub from public.financeiro_plano_contas p
     where p.cliente_id is null and p.ativo and p.ordem_exibicao in (17150, 21020, 21040, 21060, 21080, 21090, 21100, 21110, 21120, 21130, 21140, 21150, 21160, 22030);
  select count(*) into n from (select o from _lg_plano group by o having count(*) = 1) z;
  if n <> 14 or (select count(*) from _lg_plano) <> 14 then raise exception 'L3: planos de destino: % de 14', n; end if;
  ...
  select count(*), round(coalesce(sum(l.valor), 0)::numeric, 2) into n, v_s from public.financeiro_lancamentos_v2 l
   where l.cliente_id = c and l.observacao like '%LEGADO-ADM-03%';
  if n <> 766 or v_s <> 3061218.25 then raise exception 'GLOBAL: carimbados % / % (esperado 766 / 3061218.25)', n, v_s; end if;
  select count(*) into n from public.financeiro_lancamentos_v2 l join public.financeiro_plano_contas p on p.id = l.plano_conta_id
   where l.cliente_id = c and l.observacao like '%LEGADO-ADM-03%' and p.escopo_negocio = 'administrativo' and l.safra_id is not null;
