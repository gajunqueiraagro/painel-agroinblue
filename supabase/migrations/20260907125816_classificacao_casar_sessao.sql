-- 20260907125816 classificacao_casar_sessao (aplicada no proto via Management API em 07/09/2026)

ALTER TABLE public.financeiro_classificacao_staging ADD COLUMN IF NOT EXISTS excel_data_pagamento date;
ALTER TABLE public.financeiro_classificacao_staging ADD COLUMN IF NOT EXISTS excel_data_vencimento date;
ALTER TABLE public.financeiro_classificacao_staging ADD COLUMN IF NOT EXISTS casamento_meta jsonb;
ALTER TABLE public.financeiro_classificacao_staging DROP CONSTRAINT IF EXISTS financeiro_classificacao_staging_match_status_check;
ALTER TABLE public.financeiro_classificacao_staging ADD CONSTRAINT financeiro_classificacao_staging_match_status_check CHECK (match_status = ANY (ARRAY['exato','ambiguo','sem_match','ja_classificado','divergente','ambiguo_resolvido','ja_aplicado','sem_conta_para_match','candidatos_proximos','resolvido_manual','resolvido_grupo','sugestao_grupo','sugestao_split']));
...
END;
$fn$;
REVOKE ALL ON FUNCTION public.fn_classificacao_casar_sessao(uuid,text) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.fn_classificacao_casar_sessao(uuid,text) TO authenticated;
