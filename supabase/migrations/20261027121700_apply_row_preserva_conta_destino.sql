-- Aplicada no proto em 16/09/2026 pelo Chat. md5 57c1da1e -> 8f40915d.
--
-- A RPC zerava `conta_destino_id` sempre que o tipo efetivo nao era transferencia
-- (`ELSE NULL`, incondicional). Numa ENTRADA a conta mora exatamente ali — 1.392 entradas
-- deste cliente tem `conta_bancaria_id` nulo —, entao bastava apertar Salvar na Mesa para a
-- linha perder a conta. Em 16/09 isso zerou 20 entradas ja conciliadas ao OFX (Bradesco 11,
-- BB 5, Itau 4), restauradas pela migration 20261027121600.
--
-- ⚠ A ASSIMETRIA DENUNCIAVA O DEFEITO: na MESMA instrucao, `conta_bancaria_id` ja era
-- protegido por `CASE WHEN v_conciliado THEN conta_bancaria_id ELSE ... END`. A regra de
-- "conciliado nao se mexe" estava escrita para uma coluna e esquecida na outra. A troca abaixo
-- so repoe a simetria: conciliado nao perde conta; nao conciliado so muda se a proposta
-- trouxer valor. O tipo deixa de mandar na coluna.
--
-- Base: 20260909180123_fn_classificacao_apply_row_tipo_e_destino.sql, copiada byte a byte com
-- UMA substituicao (a ancora casou exatamente uma vez). Corpo conferido por md5 nas duas pontas.
...
BEGIN
  ...
  IF p_overwrite THEN
    UPDATE financeiro_lancamentos_v2 SET subcentro = COALESCE(v_proposto->>'subcentro', subcentro), macro_custo = COALESCE(v_proposto->>'macro_custo', macro_custo),
      ...
      conta_destino_id = CASE WHEN v_conciliado THEN conta_destino_id
                              ELSE COALESCE(NULLIF(v_proposto->>'conta_destino_id','')::uuid, conta_destino_id) END,
