-- PR-OC-VENDA-REALIZADO-01A — a lista branca de `oc_salvar_boitel` aceita as flags.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. O arquiteto aplicou esta expansao no Proto sob o
-- registro 20260830181424; este arquivo a guarda no repositorio.
--
-- POR QUE ELA PRECISOU EXISTIR. A migration irma (20260830181000) criou as cinco colunas,
-- e isso NAO BASTAVA: esta RPC nao escreve a tabela por reflexao — ela ENUMERA as colunas
-- uma a uma, duas vezes (na lista do INSERT/VALUES e de novo no ON CONFLICT DO UPDATE
-- SET, cada uma com o seu `CASE WHEN p_payload ? 'x'`). Medido antes de tocar no front:
-- as cinco colunas novas tinham ZERO mencoes no corpo, contra 32 nomes na lista do
-- INSERT. Um payload com as chaves novas seria descartado EM SILENCIO.
--
-- ⚠ E O SILENCIO SERIA DIFICIL DE ENXERGAR: o evento de auditoria grava o payload
-- INTEIRO (`jsonb_build_object('cenario', p_cenario, 'payload', p_payload)`), entao a
-- auditoria mostraria `custo_frete_no_boitel: true` enquanto a tabela seguia `false`. A
-- tela pareceria ter gravado.
--
-- ⚠ O `COALESCE` NOS TRES `NOT NULL` NAO E DECORACAO: as colunas recusam nulo, e um
-- payload com string vazia estouraria a escrita. Os defaults aqui repetem os da tabela,
-- de proposito — quem manda a chave sem valor recebe o mesmo que quem nao manda a chave.
-- `data_abate` e' anulavel e por isso segue o idioma de `data_envio`.
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO, e nao colada do banco: o corpo saiu do arquivo
-- 20260829240000_pr_oc_venda_boitel_rpc_01.sql (md5 do corpo 1b81f10c..., identico ao que
-- estava aplicado) com as TRES insercoes acima. Conferido pelo oraculo — o md5 do corpo
-- reconstruido e' 268525abdfbad4d5950bcbaa26bc0319, byte a byte igual ao aplicado, com
-- 14066 caracteres. As demais 29 colunas nao foram tocadas.
...
BEGIN
  ...
  INSERT INTO public.zoo_operacao_boitel AS b (
    ...
    custo_frete_no_boitel, despesas_abate_no_boitel, custo_notas_envio, notas_envio_no_boitel, data_abate,
