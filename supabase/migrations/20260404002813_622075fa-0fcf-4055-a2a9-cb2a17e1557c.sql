...
mov_meta AS (
    SELECT fazenda_id, cliente_id,
        ...
        sum(CASE WHEN tipo IN ('abate','venda','venda_pe','transferencia_saida','consumo','morte') THEN quantidade::numeric * COALESCE(peso_medio_kg, peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_saidas
    FROM lancamentos
    WHERE cancelado = false AND tipo <> 'reclassificacao' AND status_operacional = 'previsto'
    GROUP BY fazenda_id, cliente_id, EXTRACT(year FROM data::date)::integer, EXTRACT(month FROM data::date)::integer
),
mov_all AS (
...
SELECT r.fazenda_id, r.cliente_id, r.ano, r.mes, r.cenario,
    ...
    r.cab_final::integer AS cabecas_final,
    ...
    -- Peso médio: quando há fechamento, usar cab_pesado do próprio fechamento (= Valor do Rebanho)
    CASE
        WHEN pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0
            THEN round(pr.peso_total_real / pr.cab_pesado, 2)
        WHEN r.cab_final > 0
    ...
    CASE
        WHEN ((COALESCE(r.cab_inicio_mes, 0) + COALESCE(r.cab_final, 0)) / 2.0) > 0
            ...
            THEN round(
                (COALESCE(pr.peso_total_real, r.peso_final_contabil) - r.peso_inicio_contabil - COALESCE(r.peso_entradas, 0) + COALESCE(r.peso_saidas, 0))
    ...
    round(COALESCE(pr.peso_total_real, r.peso_final_contabil) - r.peso_inicio_contabil - COALESCE(r.peso_entradas, 0) + COALESCE(r.peso_saidas, 0), 2) AS gmd_numerador_kg,
    -- UA média
    CASE
        WHEN ((COALESCE(r.cab_inicio_mes, 0) + COALESCE(r.cab_final, 0)) / 2.0) > 0
             AND COALESCE(pr.cab_pesado, r.cab_final) > 0
             AND COALESCE(pr.peso_total_real, r.peso_final_contabil) > 0
            THEN round(
                (COALESCE(r.cab_inicio_mes, 0) + COALESCE(r.cab_final, 0)) / 2.0
                * (COALESCE(pr.peso_total_real, r.peso_final_contabil) / COALESCE(pr.cab_pesado, r.cab_final))
                / 450.0,
            2)
