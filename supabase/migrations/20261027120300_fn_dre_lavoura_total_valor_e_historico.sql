...
-- 1) fn_dre_lavoura: centros[].total ganha 'valor' (direto + rateado) — o front deixa de somar.
--    md5(pg_get_functiondef) esperado: d21aaaf60cbf0a1538ead9ea0e73c5bf (len 13740); md5(prosrc) 5d4ebe085bc65a12ddfb025a81eca51c.
-- 2) fn_dre_lavoura_historico: nova. md5(pg_get_functiondef) c40e01051bb9bd44466266f0c3181393 (len 2180); md5(prosrc) 6dba6d675a1e436f42e6356f7c08a048.
-- Conferir contra pg_proc, nao transcrever.
...
colheita AS (
  SELECT a.cultura, coalesce(sum(c.sacas_boas + c.grao_roca_sacas),0)::numeric AS producao
...
-- Historico por cultura: uma linha por safra em que a cultura teve area plantada, com /ha, /unidade,
-- equilibrio e a diferenca do resultado/ha contra a safra anterior. Chama fn_dre_lavoura por safra.
CREATE OR REPLACE FUNCTION public.fn_dre_lavoura_historico(p_cliente_id uuid, p_cultura text)
RETURNS jsonb LANGUAGE sql STABLE SET search_path TO 'public' AS $fn$
WITH safras AS (
  SELECT s.id, s.codigo, s.data_inicio FROM financeiro_safras s
   WHERE s.cliente_id = p_cliente_id AND s.escopo_negocio = 'agricultura'
     AND EXISTS (SELECT 1 FROM agri_safra_area a WHERE a.safra_id = s.id AND a.cliente_id = p_cliente_id AND a.ativo AND a.status = 'plantada' AND a.cultura = p_cultura)
),
d AS (
  SELECT s.codigo, s.data_inicio, c
    FROM safras s
    CROSS JOIN LATERAL (SELECT public.fn_dre_lavoura(p_cliente_id, s.id) AS j) x
    CROSS JOIN LATERAL jsonb_array_elements(x.j->'culturas') c
   WHERE c->>'cultura' = p_cultura
),
r AS (
  SELECT codigo, data_inicio,
    (c->>'area_ha')::numeric AS area_ha, (c->>'producao')::numeric AS producao, (c->>'produtividade')::numeric AS produtividade,
    (c->>'custo_operacional')::numeric AS custo_operacional,
    (c->'linhas'->'resultado_caixa'->>'valor')::numeric AS resultado,
    (c->'equilibrio'->>'preco_realizado')::numeric AS preco_realizado,
    (c->'equilibrio'->>'preco_equilibrio')::numeric AS preco_equilibrio,
    (c->'equilibrio'->>'produtividade_equilibrio')::numeric AS produtividade_equilibrio
  FROM d
),
c1 AS (
  SELECT *,
    CASE WHEN area_ha > 0 THEN round(custo_operacional/area_ha,2) END AS custo_ha,
    CASE WHEN producao > 0 THEN round(custo_operacional/producao,2) END AS custo_unidade,
    CASE WHEN area_ha > 0 THEN round(resultado/area_ha,2) END AS resultado_ha
  FROM r
),
c2 AS (
  SELECT *, resultado_ha - lag(resultado_ha) OVER (ORDER BY data_inicio, codigo) AS delta_resultado_ha FROM c1
)
SELECT coalesce(jsonb_agg(jsonb_build_object(
  'safra', codigo, 'data_inicio', data_inicio, 'area_ha', area_ha, 'producao', producao, 'produtividade', produtividade,
  'custo_ha', custo_ha, 'custo_unidade', custo_unidade, 'preco_realizado', preco_realizado, 'preco_equilibrio', preco_equilibrio,
  'produtividade_equilibrio', produtividade_equilibrio, 'resultado_ha', resultado_ha, 'delta_resultado_ha', delta_resultado_ha
) ORDER BY data_inicio, codigo), '[]'::jsonb) FROM c2;
$fn$;
GRANT EXECUTE ON FUNCTION public.fn_dre_lavoura_historico(uuid, text) TO authenticated;
