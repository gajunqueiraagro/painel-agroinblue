...
WITH categorias AS (
    ...
    SELECT pre_final.*,
           -- Cross-year LAG: partition WITHOUT ano
           ...
           lag(pre_final.ano) OVER w_cat AS prev_ano
    FROM pre_final
...
SELECT fazenda_id, cliente_id, ano, mes, cenario,
       ...
       -- saldo_inicial:
       -- Cross-year (prev_ano < ano): ALWAYS use prev_sf (chain Dec->Jan unconditionally)
       -- Same year: use governance rule (only inherit from fechamento)
       -- No previous: use cab_ini from saldos_iniciais
       CASE
           ...
           WHEN prev_sf IS NOT NULL THEN cab_inicio_mes::integer
           ELSE cab_ini::integer
       ...
       CASE
           ...
           THEN round(
               ...
               /
               (CASE
                   WHEN prev_sf IS NOT NULL AND prev_ano IS NOT NULL AND prev_ano < ano THEN prev_sf::numeric
                   WHEN prev_sf IS NOT NULL AND prev_fonte = 'fechamento' THEN prev_sf::numeric
                   WHEN prev_sf IS NOT NULL THEN cab_inicio_mes
                   ELSE cab_ini::numeric
               END)
           , 2)
           ELSE NULL::numeric
       ...
       -- GMD using the correct saldo_inicial
       CASE
           WHEN ((CASE
               WHEN prev_sf IS NOT NULL AND prev_ano IS NOT NULL AND prev_ano < ano THEN prev_sf::numeric
               WHEN prev_sf IS NOT NULL AND prev_fonte = 'fechamento' THEN prev_sf::numeric
