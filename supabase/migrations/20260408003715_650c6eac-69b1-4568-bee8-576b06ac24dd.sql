...
WITH saldo_ini AS (
    ...
    SELECT l.fazenda_id,
        ...
        sum(CASE WHEN l.tipo = ANY (ARRAY['nascimento','compra','transferencia_entrada']) THEN l.quantidade ELSE 0 END) AS entradas,
        sum(CASE WHEN l.tipo = ANY (ARRAY['abate','venda','venda_pe','transferencia_saida','consumo','morte']) THEN l.quantidade ELSE 0 END) AS saidas,
        sum(CASE WHEN l.tipo = ANY (ARRAY['nascimento','compra','transferencia_entrada']) THEN l.quantidade::numeric * COALESCE(l.peso_medio_kg, l.peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_entradas,
        sum(CASE WHEN l.tipo = ANY (ARRAY['abate','venda','venda_pe','transferencia_saida','consumo','morte']) THEN l.quantidade::numeric * COALESCE(l.peso_medio_kg, l.peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_saidas
    FROM lancamentos l
    ...
    SELECT g.fazenda_id, g.cliente_id, g.ano, g.cenario, g.mes,
        COALESCE(si.cab_ini, 0)::numeric + sum(COALESCE(mv.entradas, 0) - COALESCE(mv.saidas, 0)) OVER (PARTITION BY g.fazenda_id, g.cliente_id, g.ano, g.cenario ORDER BY g.mes ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS cab_inicio_mes,
        COALESCE(si.cab_ini, 0)::numeric + sum(COALESCE(mv.entradas, 0) - COALESCE(mv.saidas, 0)) OVER (PARTITION BY g.fazenda_id, g.cliente_id, g.ano, g.cenario ORDER BY g.mes ROWS UNBOUNDED PRECEDING) AS cab_final,
    ...
    WHERE fp.status = 'fechado' AND p.ativo = true AND p.entra_conciliacao = true
...
SELECT r.fazenda_id,
    ...
    r.dias_mes,
    CASE
        WHEN r.dias_mes > 0 AND COALESCE(CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END, 0) > 0
        THEN round(
            ...
            / ((COALESCE(r.cab_inicio_mes, 0) + COALESCE(CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END, 0)) / 2.0)
            / r.dias_mes::numeric, 4)
        ELSE NULL::numeric
    END AS gmd_kg_cab_dia,
    CASE
        WHEN COALESCE(CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END, 0) > 0
        THEN round(
            CASE WHEN r.cenario = 'realizado' AND pr.peso_total_real IS NOT NULL THEN pr.peso_total_real ELSE r.peso_final_contabil END
    ...
    CASE
        WHEN COALESCE(a.area_produtiva_ha, 0) > 0 AND COALESCE(CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END, 0) > 0
        THEN round(
            (COALESCE(r.cab_inicio_mes, 0) + COALESCE(CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END, 0)) / 2.0
            * (CASE WHEN r.cenario = 'realizado' AND pr.peso_total_real IS NOT NULL THEN pr.peso_total_real ELSE r.peso_final_contabil END
               / CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END)
