...
saldo_ini_cat AS (
  SELECT si.fazenda_id, si.cliente_id, si.ano,
    cr.id AS categoria_id, cr.codigo AS categoria_codigo, cr.nome AS categoria_nome, cr.ordem_exibicao,
...
-- GMD planejado por categoria/mês (INPUT da META)
meta_gmd AS (
...
running AS (
  SELECT fazenda_id, cliente_id, ano, mes, cenario,
    ...
    cab_ini, peso_ini,
    entradas_ext, saidas_ext, evol_cat_entrada, evol_cat_saida,
    ...
    CASE
      ...
      ELSE round(
        ...
        + COALESCE(sum(r.peso_entradas_ext - r.peso_saidas_ext + r.peso_evol_entrada - r.peso_evol_saida)
            OVER (PARTITION BY r.fazenda_id, r.ano, r.cenario, r.categoria_id ORDER BY r.mes ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 0::numeric)
        + COALESCE(sum(
            COALESCE(mg.gmd_previsto, 0::numeric)
            * ((r.cab_inicio_mes + r.cab_final) / 2.0)
            * EXTRACT(day FROM make_date(r.ano, r.mes, 1) + interval '1 month' - interval '1 day')::numeric
          ) OVER (PARTITION BY r.fazenda_id, r.ano, r.cenario, r.categoria_id ORDER BY r.mes ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW), 0::numeric)
      , 2)
    END AS ptf,
    CASE
...
SELECT fazenda_id, cliente_id, ano, mes, cenario,
  ...
  categoria_id, categoria_codigo, categoria_nome, ordem_exibicao,
  -- saldo_inicial (cabeças)
  CASE
    WHEN mes = 1 THEN cab_ini::int
    WHEN prev_fonte = 'fechamento' THEN prev_sf
    ELSE cab_inicio_mes::int
  ...
  sf AS saldo_final,
  -- peso_total_inicial: para META usa prev_ptf (que inclui prod. biológica acumulada)
  CASE
  ...
  -- peso_medio_inicial
  CASE
    WHEN (CASE WHEN mes = 1 THEN cab_ini::int WHEN prev_fonte = 'fechamento' THEN prev_sf ELSE cab_inicio_mes::int END) > 0
    THEN round(
      (CASE
        WHEN mes = 1 THEN round(peso_ini, 2)
        WHEN prev_fonte = 'fechamento' THEN prev_ptf
        ...
        ELSE round(peso_contabil_inicio_mes, 2)
      END)
      / (CASE WHEN mes = 1 THEN cab_ini::numeric WHEN prev_fonte = 'fechamento' THEN prev_sf::numeric ELSE cab_inicio_mes END)
    , 2)
    ELSE NULL::numeric
  ...
  -- gmd: usa peso_total_inicial atualizado para meta
  CASE
    WHEN ((CASE WHEN mes = 1 THEN cab_ini::numeric WHEN prev_fonte = 'fechamento' THEN prev_sf::numeric ELSE cab_inicio_mes END + sf::numeric) / 2.0) > 0
