-- 20260908143835 financiamento_pagar_pelo_extrato (aplicada no proto via Management API em 08/09/2026)
...
DECLARE v_uid uuid; cru financeiro_lancamentos_v2%ROWTYPE; p financiamento_parcelas%ROWTYPE; f financiamentos%ROWTYPE;
        v_conta uuid; v_juros numeric; v_rec jsonb; v_lp uuid; v_lj uuid; v_ci conciliacao_bancaria_itens%ROWTYPE; n_vinc int;
BEGIN
  ...
  VALUES (cru.cliente_id, v_ci.extrato_id, v_lp, p.valor_principal, v_uid, v_ci.snapshot_extrato_valor, p.valor_principal, v_ci.snapshot_extrato_data, cru.data_pagamento, v_ci.snapshot_historico_banco, 'financiamento_pelo_extrato', v_uid, now(), COALESCE(v_ci.grupo_id, gen_random_uuid()));
  IF v_lj IS NOT NULL AND v_juros > 0 THEN
    INSERT INTO conciliacao_bancaria_itens (cliente_id, extrato_id, lancamento_id, valor_aplicado, criado_por, snapshot_extrato_valor, snapshot_lancamento_valor, snapshot_extrato_data, snapshot_lancamento_data, snapshot_historico_banco, tipo_aprovacao, aprovado_por, aprovado_em, grupo_id)
    VALUES (cru.cliente_id, v_ci.extrato_id, v_lj, v_juros, v_uid, v_ci.snapshot_extrato_valor, v_juros, v_ci.snapshot_extrato_data, cru.data_pagamento, v_ci.snapshot_historico_banco, 'financiamento_pelo_extrato', v_uid, now(), COALESCE(v_ci.grupo_id, gen_random_uuid()));
  END IF;
