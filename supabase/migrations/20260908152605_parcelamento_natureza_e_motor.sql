-- PARC-DB-01 (08/09/2026): natureza do contrato e motor de parcela ciente de parcelamento
ALTER TABLE public.financiamentos ADD COLUMN IF NOT EXISTS natureza text NOT NULL DEFAULT 'financiamento';
ALTER TABLE public.financiamentos DROP CONSTRAINT IF EXISTS chk_financiamentos_natureza;
ALTER TABLE public.financiamentos ADD CONSTRAINT chk_financiamentos_natureza CHECK (natureza IN ('financiamento','parcelamento','emprestimo'));
COMMENT ON COLUMN public.financiamentos.natureza IS 'PARC-DB-01: financiamento (bem vinculado), emprestimo (sem bem) ou parcelamento (despesa em N vezes, sem juros, principal na classificacao da parcela). Escopo continua em tipo_financiamento.';
...
BEGIN
  ...
  -- PARC-DB-01 (08/09/2026): parcelamento e despesa paga em N vezes. O principal vai para a
  -- classificacao escolhida no contrato (plano_conta_parcela_id) e nao para Amortizacao; nao ha
  -- juros (o cadastro grava valor_juros = 0 e o ramo de juros abaixo so roda com valor_juros > 0).
  IF v_contrato.natureza = 'parcelamento' THEN
    IF v_contrato.plano_conta_parcela_id IS NULL THEN
      v_alertas := v_alertas || jsonb_build_object('tipo','parcelamento_sem_plano_conta_parcela',
        'financiamento_id', v_contrato.id);
    ELSE
      v_plano_p_id := v_contrato.plano_conta_parcela_id;
    END IF;
  END IF;
  ...
  v_data_pag := CASE
    ...
    ELSE NULL::date
  END;
  v_desc_p := CASE WHEN v_contrato.natureza = 'parcelamento'
    THEN 'Parcela ' || v_parcela.numero_parcela::text || '/' || COALESCE(v_contrato.total_parcelas::text,'?') || ' ' || COALESCE(v_contrato.descricao,'')
    ELSE 'Amortiza' || chr(231) || chr(227) || 'o ' || COALESCE(v_contrato.descricao,'') || ' ' || COALESCE(v_contrato.numero_contrato,'') END;
...
BEGIN
  ...
  -- PARC-DB-01: parcelamento nao tem juros; a parcela assume o valor do extrato como principal.
  IF f.natureza = 'parcelamento' THEN
    UPDATE financiamento_parcelas SET valor_principal = cru.valor WHERE id = p.id AND NOT p_simular;
    p.valor_principal := cru.valor;
    v_juros := 0;
  END IF;
