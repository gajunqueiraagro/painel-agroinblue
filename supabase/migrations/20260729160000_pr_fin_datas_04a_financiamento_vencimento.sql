-- PR-FIN-DATAS-04A — Writer OFICIAL de financiamento passa a preservar o VENCIMENTO.
--   CREATE OR REPLACE da RPC fn_reconciliar_parcela_financiamento (writer LIVE de
--   financeiro_lancamentos_v2 a partir de financiamento_parcelas). Base VERBATIM da versão
--   vigente — âncora: 20260706_pr_fin_cpr01_conta_parcela.sql (linhas 37-647). ÚNICA mudança:
--   propagar financiamento_parcelas.data_vencimento -> financeiro_lancamentos_v2.data_vencimento
--   nos lançamentos Principal e Juros (criação + reconciliação/atualização), com deteção
--   NULL-safe (IS DISTINCT FROM). A parcela é a fonte soberana do vencimento; correção manual
--   divergente no lançamento é ressincronizada (mesmo contrato dos demais campos).
--   NÃO altera data_pagamento/data_competencia/ano_mes/status_transacao, valores, contas,
--   classificação, vínculos, cancelamento, proteções nem a assinatura da função. Sem backfill.
--   Assinatura preservada: (p_parcela_id, p_dry_run, p_recalcula_vt, p_conta_bancaria_id).
--   Alvo: PROTO (binbcdfbisgscrifztia). NÃO aplicada nesta fase.
      ...
      LOOP
        ...
        IF v_acao_tipo = 'criar_principal' THEN
          ...
          INSERT INTO financeiro_lancamentos_v2 (
            valor, descricao, data_pagamento, data_competencia, data_vencimento, ano_mes, status_transacao,
