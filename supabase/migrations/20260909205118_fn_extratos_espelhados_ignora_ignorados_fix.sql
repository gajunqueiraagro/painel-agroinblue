-- PR-CONCIL-ESPELHO — extrato desconsiderado sai do espelho.
--
-- O espelho filtrava so' o extrato CANCELADO; o desconsiderado (`ignorado_em`)
-- continuava listado e somado, e a duplicidade da provisao de cartao reaparecia
-- como sem par. Uma linha no CTE que le `extrato_bancario_v2`.
--
-- Derivada da definicao versionada em 20260909131053 (md5 do corpo
-- 6547d9e9002017cee34854c4b098ab3a, 4761) + UMA linha apos
-- "    AND e.cancelado_em IS NULL". Corpo resultante:
--   md5 d05ec07eda440ff38f16075e161c31be, 4791 — conferido no pg_proc.
--
-- ⚠ A 20260909202753 NAO EXECUTA NADA. Ela foi aplicada 24 minutos antes com a
--    troca errada — duplicou "AND e.data_movimento BETWEEN pp.d1 AND pp.d2" em vez
--    de acrescentar o filtro de `ignorado_em` (medido no pg_proc: zero ocorrencias
--    de ignorado_em, duas do filtro de data, e o delta de 49 caracteres era
--    exatamente a linha repetida). Esta migration a sobrescreve e parte da 131053
--    limpa, entao a duplicata nao sobrevive. Ela ganha um arquivo SO' COM COMENTARIO,
--    para o historico local casar com o remoto sem fingir que fez alguma coisa.
...
lv2 AS (
  SELECT l.id, l.data_pagamento AS dt,
         ...
         l.origem_lancamento, l.favorecido_id, l.data_competencia,
...
SELECT jsonb_build_object(
  ...
  'sistema_completo',(SELECT COALESCE(jsonb_agg(jsonb_build_object(
            ...
            'origem_lancamento',origem_lancamento,'favorecido_id',favorecido_id,'fornecedor',fornecedor,'competencia',data_competencia) ORDER BY dt, id),'[]'::jsonb) FROM sis_status),
  'versao','espelhados-03-competencia',
  'gerado_em', now()
)
$function$;
