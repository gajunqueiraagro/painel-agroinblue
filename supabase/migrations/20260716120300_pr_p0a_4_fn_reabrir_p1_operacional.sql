-- PR-P1-GOV-REABERTURA-P0A — Migration 4: fn_reabrir_p1_operacional (RPC idempotente)
-- Reabertura transacional P1: invalida P2 realizado (mes alvo + cadeia), abre P2
--   fechamento do alvo, reabre cards de pastos (fechado->rascunho; bypass do guard),
--   e mantem cabecalho fechamento_p1. Idempotente: 2a execucao nao gera efeito.
-- SECURITY DEFINER, search_path=public. Guard: auth.uid() + membership SETOF-inequivoca
--   (alias t(cliente_id)) ou admin. Sem anon/PUBLIC; grant apenas authenticated.
--
-- GATE DE ELEGIBILIDADE DO ALVO (obrigatorio): a cascata so pode ocorrer se o mes-alvo
--   possui ao menos um estado legitimo de reabertura. Sem isso, chamar a RPC para um
--   mes arbitrario (ex.: '2019-01' inexistente) invalidaria meses futuros validados.
--   Elegivel quando, no alvo, houver: realizado validado, OU P2 fechamento fechado, OU
--   algum card de pasto fechado, OU cabecalho fechamento_p1 em estado oficial/reabrivel
--   ('oficial'/'oficializando'). Se inelegivel: no-op total (nada alterado).
-- Validacao estrita de p_ano_mes (YYYY-MM, mes 01-12) ANTES de qualquer efeito ou
--   comparacao textual, para nunca deixar string arbitraria participar de ano_mes > p_ano_mes.
-- Auditoria deriva SOMENTE de UPDATE ... RETURNING (CTE afetados). Booleano
--   v_houve_mudanca de contagens explicitas (nunca NULL). Cabecalho tocado apenas se
--   houve mudanca. Ordem: invalidar P2 realizado ANTES de reabrir cards, de modo que o
--   trigger AFTER UPDATE (corrigido em B0) encontre status 'invalidado' e nao re-execute
--   cascata/log.

CREATE OR REPLACE FUNCTION public.fn_reabrir_p1_operacional(
  p_fazenda_id uuid, p_ano_mes text, p_motivo text)
RETURNS jsonb
...
REVOKE EXECUTE ON FUNCTION public.fn_reabrir_p1_operacional(uuid, text, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.fn_reabrir_p1_operacional(uuid, text, text) FROM anon;
GRANT EXECUTE ON FUNCTION public.fn_reabrir_p1_operacional(uuid, text, text) TO authenticated;
