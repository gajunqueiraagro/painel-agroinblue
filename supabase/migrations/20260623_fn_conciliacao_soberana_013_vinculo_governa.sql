...
--                           (cancelado | sinal_cruzado | conta_divergente |
--                            valor_divergente | data_divergente).
...
ofx_valid AS (
  SELECT DISTINCT extrato_id FROM links_raw WHERE motivo='valido' AND ofx_status <> 'ignorado'
),
...
div AS (
  ...
  WHERE lr.motivo <> 'valido' AND lr.ofx_status <> 'ignorado'
...
corretos AS (
  SELECT DISTINCT extrato_id, ofx_v FROM links_raw WHERE motivo='valido' AND ofx_status <> 'ignorado'
),
...
SELECT jsonb_build_object(
  'gerado_em', now(),
  'versao', 'soberana-01.3-vinculo-governa',
