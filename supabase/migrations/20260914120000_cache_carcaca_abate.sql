-- PR-CACHE-CARCACA-01 — a carcaca do abate entra no cache.
--
-- POR QUE. A regra canonica da arroba (calcArrobas, economicos.ts:37) usa
-- carcaca/15 no abate e peso vivo/30 em todo o resto. O cache so tinha peso
-- vivo, entao a linha "Abates" do bloco @ subestimaria:
--   565 de 565 abates realizados tem carcaca (100%)
--   por carcaca    439.605,2 @
--   por peso vivo  411.950,9 @
--   diferenca       27.654,3 @  (6,7%)
-- Alem de errado, divergiria do desfrute_arr que o resto do sistema exibe.
--
-- O BLOCO @ NAO CALCULA POR LANCAMENTO. O cache existe para o PC-100 nao
-- varrer `lancamentos` — foi o que a 20260913120000 estabeleceu, e esta
-- migration completa aquela.
--
-- `peso_abate` (peso VIVO) permanece intacto. As duas colunas convivem: uma
-- serve ao bloco kg, a outra ao bloco @.
--
-- MESMA ESTRUTURA da 20260913120000, agora com UMA coluna. Os mesmos nove
-- pontos de propagacao, e o mesmo ponto de risco: o segundo ramo do UNION ALL
-- de `mov_all` e POSICIONAL, sem alias. A coluna entra sempre LOGO APOS
-- `peso_morte`, nos nove — conferido por extracao antes do commit.
--
-- NAO RECONSTROI O CACHE. Rebuild e operacao separada.
...
-- PASSO 1 — a coluna.
-- ─────────────────────────────────────────────────────────────────────────
/* Carcaca do abate no cache: a regra canonica da arroba
   (calcArrobas, economicos.ts:37) usa carcaca/15 no abate e peso vivo/30
   no resto. Sem esta coluna a linha "Abates" em @ subestimaria 6,7%
   (439.605 contra 411.951 @, medido em 22/08) e divergiria do
   desfrute_arr. */
ALTER TABLE public.zoot_mensal_cache
  ADD COLUMN peso_carcaca_abate numeric;

-- ─────────────────────────────────────────────────────────────────────────
-- PASSO 4 — COMMENT.
-- ─────────────────────────────────────────────────────────────────────────
COMMENT ON COLUMN public.zoot_mensal_cache.peso_carcaca_abate IS
  'Peso de CARCACA dos abates do mes, em kg. Base da arroba do abate (carcaca/15). Distinto de peso_abate, que e peso VIVO. Sem fallback: carcaca ausente soma zero.';

-- ─────────────────────────────────────────────────────────────────────────
-- PASSO 2 — fn_zoot_categoria_mensal.
--
-- DROP + CREATE de novo: RETURNS TABLE muda, e OR REPLACE nao aceita.
-- ─────────────────────────────────────────────────────────────────────────
...
-- PASSO 5 — ACL.
-- ─────────────────────────────────────────────────────────────────────────
/* REVOKE obrigatorio: DROP+CREATE restaura o default do Postgres, que e
   EXECUTE para PUBLIC. Aconteceu na migration 20260913120000 — a ACL
   voltou com =X/postgres numa funcao SECURITY DEFINER, e `anon` passou a
   alcancar. Foi revogado a mao; desta vez vai na migration. */
...
/* Os DOIS papeis, conferidos no banco em 22/08 DEPOIS da 20260913120000:
   postgres=X/postgres | service_role=X/postgres | authenticated=X/postgres.
   `service_role` nao existia antes daquela migration — foi concedido a mao
   junto com o REVOKE, e restaurar so `authenticated` o perderia. */
GRANT EXECUTE ON FUNCTION public.fn_zoot_categoria_mensal(uuid, integer, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_zoot_categoria_mensal(uuid, integer, text) TO service_role;

-- ─────────────────────────────────────────────────────────────────────────
-- PASSO 3 — as TRES overloads de refresh_zoot_cache. Seis listas.
-- CREATE OR REPLACE: retorno `void` nao muda, entao nao perdem ACL.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ROLLBACK (comentado)
--
-- A migration faz DROP da funcao: sem este bloco nao ha volta. O texto e o
-- pg_get_functiondef() de 22/08/2026 APOS a 20260913120000, 22.604
-- caracteres, conferido byte a byte contra o banco antes de ser copiado.
--
-- ORDEM OBRIGATORIA: refresh volta ANTES do DROP COLUMN, senao fica
-- referenciando coluna inexistente.
--
-- BEGIN;
--
-- -- 1. as tres refresh_zoot_cache, sem a coluna nova
...
--
-- -- 3. por fim, a coluna
-- ALTER TABLE public.zoot_mensal_cache DROP COLUMN peso_carcaca_abate;
--
-- COMMIT;
