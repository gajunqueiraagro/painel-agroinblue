...
-- Fix BOTH views: filter only active pastos (ativo=true)

-- 1. Fix vw_zoot_categoria_mensal
...
-- CRITICAL FIX: Only count fechamentos from ACTIVE pastos (ativo=true)
peso_real_cat AS (
...
-- 2. Fix vw_zoot_fazenda_mensal - same issue
...
mov_realizado AS (
    SELECT fazenda_id, cliente_id,
        ...
        sum(CASE WHEN tipo IN ('nascimento','compra','transferencia_entrada') THEN quantidade::numeric * COALESCE(peso_medio_kg, peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_entradas,
        sum(CASE WHEN tipo IN ('abate','venda','venda_pe','transferencia_saida','consumo','morte') THEN quantidade::numeric * COALESCE(peso_medio_kg, peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_saidas
    FROM lancamentos
    WHERE cancelado = false AND tipo <> 'reclassificacao' AND COALESCE(status_operacional, 'conciliado') = 'conciliado'
...
mov_meta AS (
    SELECT fazenda_id, cliente_id,
        ...
        sum(CASE WHEN tipo IN ('abate','venda','venda_pe','transferencia_saida','consumo','morte') THEN quantidade::numeric * COALESCE(peso_medio_kg, peso_carcaca_kg, 0::numeric) ELSE 0::numeric END) AS peso_saidas
    FROM lancamentos
    WHERE cancelado = false AND tipo <> 'reclassificacao' AND cenario = 'meta'
    GROUP BY fazenda_id, cliente_id, EXTRACT(year FROM data::date)::integer, EXTRACT(month FROM data::date)::integer
),
mov_all AS (
...
running AS (
    SELECT expanded.*,
        expanded.cab_ini::numeric + COALESCE(sum(expanded.entradas - expanded.saidas) OVER w_prev, 0::numeric) AS cab_inicio_mes,
        expanded.peso_ini + COALESCE(sum(expanded.peso_entradas - expanded.peso_saidas) OVER w_prev, 0::numeric) AS peso_inicio_contabil,
        expanded.cab_ini::numeric + sum(expanded.entradas - expanded.saidas) OVER w_curr AS cab_final,
        expanded.peso_ini + sum(expanded.peso_entradas - expanded.peso_saidas) OVER w_curr AS peso_final_contabil
    FROM expanded
    WINDOW w_prev AS (PARTITION BY expanded.fazenda_id, expanded.ano, expanded.cenario ORDER BY expanded.mes ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING),
           w_curr AS (PARTITION BY expanded.fazenda_id, expanded.ano, expanded.cenario ORDER BY expanded.mes ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)
),
-- CRITICAL FIX: Only count fechamentos from ACTIVE pastos (ativo=true)
peso_real AS (
    SELECT fp.fazenda_id, fp.ano_mes,
...
SELECT r.fazenda_id, r.cliente_id, r.ano, r.mes, r.cenario,
    ...
    CASE
        ...
        THEN round(
            (CASE WHEN r.cenario = 'realizado' AND pr.peso_total_real IS NOT NULL THEN pr.peso_total_real ELSE r.peso_final_contabil END
             - r.peso_inicio_contabil
             - COALESCE(r.peso_entradas, 0) + COALESCE(r.peso_saidas, 0))
            / ((COALESCE(r.cab_inicio_mes, 0) + COALESCE(
                CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END
            , 0)) / 2.0)
            / EXTRACT(day FROM make_date(r.ano, r.mes, 1) + interval '1 month' - interval '1 day')
    ...
    round((COALESCE(r.cab_inicio_mes, 0) + COALESCE(
        CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END
    , 0)) / 2.0 * 450.0 / 450.0, 2) AS ua_media,
    COALESCE(a.area_produtiva_ha, 0::numeric) AS area_produtiva_ha,
    CASE
        WHEN COALESCE(a.area_produtiva_ha, 0) > 0 THEN round(
            (COALESCE(r.cab_inicio_mes, 0) + COALESCE(
                CASE WHEN r.cenario = 'realizado' AND pr.cab_pesado IS NOT NULL AND pr.cab_pesado > 0 THEN pr.cab_pesado::numeric ELSE r.cab_final END
            , 0)) / 2.0 * 450.0 / 450.0 / a.area_produtiva_ha
        , 2)
        ELSE 0::numeric
