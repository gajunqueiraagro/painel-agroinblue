-- ═══════════════════════════════════════════════════════════════════════════════
-- PR-OC-VENDA-BOITEL-RPC-01 — a porta de escrita de `zoo_operacao_boitel`
--
-- APLICADA no Proto em 2026-08-29, com GO explicito do Gabriel.
--
-- POR QUE ELA EXISTE
-- `zoo_operacao_boitel` nasceu em PR-OC-VENDA-BOITEL-DESTINO-01 com RLS ligado, UMA
-- politica de leitura e ZERO grants. Sem grant, o PostgREST nao escreve — por desenho.
-- Esta e' a unica porta.
--
-- DE ONDE VEIO O IDIOMA
-- `oc_salvar_lotes` e' a referencia principal: e' a outra RPC que escreve uma tabela
-- FILHA da OC a partir de um payload, e dela vieram, na ordem, o guarda de autorizacao,
-- o `SELECT ... FOR UPDATE` da operacao, os guardas de `cancelada` e `fechada`, o guarda
-- de versao com ERRCODE 40001, o `versao + 1`, o evento em `zoo_operacao_eventos` e o
-- formato do envelope de retorno.
-- `oc_editar_negociacao` e' a referencia secundaria, e dela veio UMA coisa: o idioma
-- `CASE WHEN p_payload ? 'campo' THEN ... ELSE campo END`, que PRESERVA o que nao foi
-- enviado em vez de anular. Isso importa aqui mais do que la': os quatro modais do 01B
-- editam blocos diferentes da mesma linha, e o modal de Custos nao pode apagar o que o
-- de Desempenho gravou.
-- ═══════════════════════════════════════════════════════════════════════════════
...
COMMENT ON FUNCTION public.oc_salvar_boitel(uuid,uuid,integer,text,jsonb) IS
  'PR-OC-VENDA-BOITEL-RPC-01: unica porta de escrita de zoo_operacao_boitel (a tabela nao tem grants). Upsert por (operacao_id, cenario) — uma chamada de realizado nunca alcanca a linha projetada. Payload parcial PRESERVA o que nao veio. Os dois cenarios exigem dias, gmd, rendimento_saida_pct, custo_diaria e preco_venda_arroba, validados na LINHA RESULTANTE.';

-- ═══════════════════════════════════════════════════════════════════════════════
-- O QUE ESTA MIGRATION NAO FAZ
--
-- 1. NAO da grant nenhum. A RPC e' SECURITY DEFINER e a tabela segue sem grants — o
--    PostgREST alcanca a FUNCAO, nao a tabela.
--
-- 2. NAO toca em `oc_salvar_rascunho`, `oc_confirmar` nem em nenhuma RPC existente.
--
-- 3. NAO cria RPC de leitura: a politica de SELECT de `zoo_operacao_boitel` ja permite
--    ler pelo PostgREST.
--
-- 4. NAO marca nada na DRE. O asterisco de "valor a validar" nao existe e e' frente
--    propria; esta RPC apenas grava o projetado.
-- ═══════════════════════════════════════════════════════════════════════════════
