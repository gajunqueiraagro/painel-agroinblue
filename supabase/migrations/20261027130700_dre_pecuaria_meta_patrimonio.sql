-- DRE-PEC-RPC-04 (22/09/2026) - fn_dre_pecuaria: a META ganha a variacao TOTAL do valor do rebanho, como o PC-100
--
-- Corpo de partida: pg_get_functiondef do banco proto, conferido por md5 do prosrc antes de editar
-- (ae76ac4543301819b09507766dcf2c03 - a 20261027130600). Assinatura igual: CREATE OR REPLACE.
-- No cenario 'realizado' nada muda: a CTE nova (patm) e vazia e todo `case` cai no ramo de antes.
--
-- DEFEITO (homologacao do Gabriel): a coluna Meta mostrava "—" em Variacao por producao e Efeito de
-- mercado. A RPC-03 zerou o patrimonio da meta; o sistema tem, sim, o valor do rebanho planejado.
--
-- A REGRA E A DO PC-100, copiada (decisao do Gabriel em 22/09, caminho A):
--   - final   = valor_rebanho_meta_validada.valor_total do mes final, somado por fazenda, sem filtro
--               de status (PainelConsultorTab.tsx:2013-2057);
--   - inicial = em janeiro, o REALIZADO validado de dezembro do ano anterior
--               (valor_rebanho_realizado_validado, status = 'validado', PainelConsultorTab.tsx:2156); nos
--               outros meses, o final da META do mes anterior (PainelConsultorTab.tsx:1461-1462 e :2501);
--   - o PC-100 NAO decompoe a variacao da meta em producao e mercado (varValorRebMeta = final - inicial,
--     PainelConsultorTab.tsx:1473). Por isso, na meta: vpb_operacional = final - inicial (a variacao
--     TOTAL), efeito_mercado = null, v_ini_p0 e v_fim_p1 preenchidos, v_fim_p0 null, e
--     patrimonio.criterio = 'variacao total (PC-100)'. O VBP e os resultados da meta voltam a incluir
--     essa variacao - a da meta, nao a do realizado.
--   ⚠ PARA A TELA: a celula da meta nessa linha nao e "variacao por producao"; o title deve dizer
--     "Variacao total do rebanho (meta)".
-- Conferido antes de aplicar, jan-ago/26: Agnaldo -1.857.002,19; NJ -481.367,69 (iguais ao PC-100).
