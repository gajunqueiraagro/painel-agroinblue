-- PR-OC-COMPRA-NEG-01 — oc_salvar_lotes passa a persistir a BASE FINANCEIRA da negociação.
--   Decisão de negócio: valor_acordado = SOMA BRUTA dos lotes negociados (kg -> qtd*peso*valor;
--   cabeca -> qtd*valor; total -> valor). NÃO inclui frete, impostos, retenções, descontos,
--   bônus, pagamentos ou ajustes de liquidação. Sem preço válido (Σ=0) -> valor_acordado = NULL.
--   Efeito: a operação deixa de ficar base_indefinida na regra de saldo (D2: final>ACORDADO>
--   estimado>indefinida); a negociação vira a base 'acordado'. Fórmula idêntica ao front
--   (useCompraLotes.totais). Tudo o mais preservado verbatim: guardas de tenant/operação,
--   bloqueio em cancelada/fechada, lock otimista por versão, REPLACE de lotes, evento, grants.
--   NÃO cria lançamentos, NÃO toca FINV2/títulos, NÃO mexe em movimentações.
...
DECLARE
  ...
  v_ordens int[] := '{}';
  v_count int := 0;
  v_total_acordado numeric := 0;   -- PR-OC-COMPRA-NEG-01: soma bruta dos lotes -> valor_acordado
BEGIN
  ...
  -- REPLACE: apaga os lotes atuais da operacao e reinsere; acumula a soma bruta negociada.
  ...
  LOOP
    ...
    -- Total bruto do lote (mesma fórmula do front). Só compõe se houver preço informado.
  ...
  -- Bump da operacao (updated_at/by + versao) + BASE FINANCEIRA da negociacao (valor_acordado).
  --   Σ=0 (sem preço válido) -> NULL, preservando base_indefinida na regra de saldo.
  ...
  VALUES (p_cliente_id, p_operacao_id, 'salvar_lotes',
          jsonb_build_object('lotes', p_lotes, 'valor_acordado', NULLIF(v_total_acordado, 0)), v_actor, 'rpc');

  RETURN jsonb_build_object('ok', true, 'operacao_id', p_operacao_id, 'versao', v_op.versao + 1,
                            'lotes', v_count, 'valor_acordado', NULLIF(v_total_acordado, 0));
END;
$function$;
