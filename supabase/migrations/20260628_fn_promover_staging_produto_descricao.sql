-- 20260628_fn_promover_staging_produto_descricao.sql
-- TASK-001 — Produto indo para Observação (correção do caminho MERGE/UPDATE).
--
-- Bug: no MERGE (GUARD 6), o lancamento existente era enriquecido gravando
--   observacao = v_obs_rica, onde v_obs_rica concatenava Produto · descricao ·
--   observacao. Como o MERGE nao tocava descricao, o Produto caia em observacao.
--   O INSERT (orfaos) ja fazia o certo: Produto dobrado na descricao e
--   observacao recebendo apenas v_row.observacao.
--
-- Correcao (opcao A aprovada): MERGE passa a espelhar o INSERT:
--   - descricao recebe o Produto dobrado (mesma expressao do INSERT);
--     quando o Excel nada traz, preserva o descricao existente (nao destroi OFX);
--   - observacao recebe apenas v_row.observacao;
--   - v_obs_rica deixa de existir (era usada so aqui).
-- Sem coluna produto nova, sem mudanca de modelo, sem backfill, sem tocar
-- transferencias/memoria/front. Resto da funcao identico ao estado vivo
