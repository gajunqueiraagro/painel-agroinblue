-- 20260628_fn_ws_conciliacao_conta_null_t006b.sql
-- TASK-006B — Conta bancária nula legítima (lançamento zootécnico/abate).
-- ÚNICA mudança: no ramo sistema -> OFX (p_tipo='sistema_sem_vinculo'), o filtro
-- de conta dos candidatos passa a permitir lançamento com conta NULL:
--   AND e.conta_bancaria_id = v_conta
--   ->
--   AND (v_conta IS NULL OR e.conta_bancaria_id = v_conta)
-- Nenhum outro filtro alterado. Ramo OFX -> sistema (ELSE) intacto. Sem mexer
-- em score/matching, agrupamento, transferência ou D2.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'acoes_disponiveis', jsonb_build_object('vincular',false,'editar',false,'criar',false,'ignorar',false)
  );
END;
$function$;
