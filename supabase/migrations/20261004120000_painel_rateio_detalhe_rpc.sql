...
--       579a3aa03ec5f8831a4995f5b69b4d2a  (3.570 bytes)
...
--   administrativo tem `safra_id` nulo e entra por JANELA DE DATAS (`data_competencia` entre o
--   inicio e o fim da safra) vezes o percentual de `agri_rateio_admin` do ano. Quatro macros
--   ficam de fora por nao serem custeio: Dividendos, Investimento na Fazenda, Saida Financeira e
--   Transferencias.
--
...
AS $function$
declare v_pool numeric:=0; v_direto numeric:=0; v_fatias jsonb; v_lanc jsonb; v_ini date; v_fim date;
begin
  ...
  return jsonb_build_object('pool',round(v_pool,2),'direto_cultura',round(v_direto,2),'fatias',v_fatias,'lancamentos',coalesce(v_lanc,'[]'::jsonb));
end $function$;
