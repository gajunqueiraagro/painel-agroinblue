-- ESPELHO — casar N extratos : 1 lancamento preenche a conta POR DIRECAO (fn_espelho_casar_n1).
--
-- Antes o UPDATE gravava `conta_bancaria_id = COALESCE(conta_bancaria_id, v_conta)` para
-- QUALQUER sinal — uma entrada casada aqui ficava com a conta na coluna errada (entrada guarda a
-- conta em conta_destino_id). Agora: sinal '-1' (saida) -> conta_bancaria_id; sinal '1' (entrada)
-- -> conta_destino_id, so quando a coluna esta vazia; o WHERE acompanha. Lancamentos ja casados
-- nao mudam. Validacoes, soma/tolerancia 0,01, grupo, vinculos e retorno: iguais a 20260909154844.
--
-- Copiado do pg_get_functiondef do proto e conferido lendo este arquivo:
--   corpo (prosrc): md5 6bb6c7d3414df925a90e16aa8bb2af46, len 4079
--   definicao completa: md5 51c1046372092cfe5e7af52b41be80d7, len 4362
...
BEGIN
  ...
  UPDATE financeiro_lancamentos_v2
     SET data_pagamento = v_data, status_transacao = 'realizado',
         conta_bancaria_id = CASE WHEN sinal = '-1' THEN COALESCE(conta_bancaria_id, v_conta) ELSE conta_bancaria_id END,
         conta_destino_id  = CASE WHEN sinal = '1'  THEN COALESCE(conta_destino_id,  v_conta) ELSE conta_destino_id  END,
         updated_by = v_uid, updated_at = v_agora
   WHERE id = p_lancamento_id
     AND (data_pagamento IS DISTINCT FROM v_data OR status_transacao IS DISTINCT FROM 'realizado' OR (sinal = '-1' AND conta_bancaria_id IS NULL)
