-- PR-FIN-OC-CONTRATO-01 — Contrato temporal dos títulos da Operação Comercial.
-- Escopo restrito: SOMENTE o writer oficial oc_gerar_obrigacoes + backfill dos títulos OC.
-- NÃO altera writers legados (manual/OFX/importação/Mesa/contratos/financiamentos/rebanho).
-- NÃO migra os ~68k lançamentos. oc_sincronizar permanece DORMENTE e INTOCADA (Decisão 7).
--
-- Mudança de contrato no título materializado (financeiro_lancamentos_v2):
--   data_pagamento  = NULL         (obrigação nasce ABERTA; pagamento efetivo só por liquidação/conciliação — PR-2)
--   data_vencimento = v_data_pag   (vencimento resolvido = COALESCE(item.venc, oc.prevista, oc.data_operacao); nunca nulo)
--   ano_mes         = mês do vencimento (data financeira derivada; antes vinha da competência data_operacao)
-- Idempotência (chave_idempotencia sob FOR UPDATE), guardas de base/classificação/favorecido e todo o
-- restante do corpo: PRESERVADOS VERBATIM. Constraints de unicidade/identidade/FK: inalteradas.
...
BEGIN
  ...
  LOOP
    ...
    IF v_materializar THEN
      ...
      -- PR-FIN-OC-CONTRATO-01 — v_data_pag passa a ser o VENCIMENTO resolvido (nunca nulo). O título nasce
      --   ABERTO: data_pagamento=NULL; data_vencimento=v_data_pag; ano_mes = mês do vencimento (data derivada).
...
-- ============================================================================
-- BACKFILL GOVERNADO — SOMENTE títulos originados da OC, abertos, no contrato antigo,
--   sem liquidação e sem conciliação. Guarda dupla (fechamento + liquidação/conciliação).
-- ============================================================================
DO $backfill$
BEGIN
  -- Guarda 1: período financeiro fechado envolvendo título OC (ano_mes atual OU alvo).
  IF EXISTS (
    SELECT 1
      FROM financeiro_lancamentos_v2 fl
      JOIN financeiro_fechamentos f
        ON f.cliente_id = fl.cliente_id
       AND f.status_fechamento = 'fechado'
       AND f.ano_mes IN (fl.ano_mes, to_char(COALESCE(fl.data_vencimento, fl.data_pagamento),'YYYY-MM'))
     WHERE fl.origem_lancamento = 'operacao_comercial' AND fl.cancelado = false
  ) THEN
    RAISE EXCEPTION 'PR-FIN-OC-CONTRATO-01: backfill abortado — periodo fechado envolvendo titulo OC';
  END IF;

  -- Guarda 2: liquidação/conciliação em título OC aberto (não deve haver neste ciclo).
  IF EXISTS (
    SELECT 1 FROM financeiro_lancamentos_v2 fl
     WHERE fl.origem_lancamento='operacao_comercial' AND fl.cancelado=false
       AND fl.status_transacao='programado' AND fl.data_pagamento IS NOT NULL
       AND ( fl.conciliado_em IS NOT NULL
          OR EXISTS (SELECT 1 FROM financeiro_conciliacoes c WHERE c.lancamento_id=fl.id)
          OR EXISTS (SELECT 1 FROM zoo_operacao_liquidacoes l WHERE l.financeiro_lancamento_id=fl.id AND l.estornado IS NOT TRUE) )
  ) THEN
    RAISE EXCEPTION 'PR-FIN-OC-CONTRATO-01: backfill abortado — titulo OC aberto com liquidacao/conciliacao';
  END IF;

  -- Backfill: preserva id/vínculo/valor/competência/fazenda/contraparte/classificação/status/origem.
  UPDATE financeiro_lancamentos_v2 fl
     SET data_vencimento = COALESCE(fl.data_vencimento, fl.data_pagamento),
         data_pagamento  = NULL,
         ano_mes         = to_char(COALESCE(fl.data_vencimento, fl.data_pagamento),'YYYY-MM'),
         updated_at      = now()
   WHERE fl.origem_lancamento = 'operacao_comercial'
     AND fl.cancelado = false
     AND fl.status_transacao = 'programado'
     AND fl.data_pagamento IS NOT NULL
     AND NOT EXISTS (
       SELECT 1 FROM zoo_operacao_liquidacoes l
        WHERE l.financeiro_lancamento_id = fl.id AND l.estornado IS NOT TRUE);
END
$backfill$;
