-- 20260629_fn_marcar_extrato_transferencia_security_definer.sql
-- FASE 2A — promove fn_marcar_extrato_transferencia a SECURITY DEFINER + search_path
-- fixo + grants no padrão D1/D2. Corpo da lógica IDÊNTICO ao vivo (gerado por
-- pg_get_functiondef); só o cabeçalho (SECURITY DEFINER / SET search_path) e os
-- GRANTs mudam. NÃO adiciona guard de mês fechado (decisão consciente: fechamento
-- é por cliente_id+fazenda_id+ano_mes e a transferência tem fazenda_id=NULL, então
-- o guard nunca dispararia — guard de fachada).
...
REVOKE ALL ON FUNCTION public.fn_marcar_extrato_transferencia(uuid,uuid,text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.fn_marcar_extrato_transferencia(uuid,uuid,text) FROM anon;
GRANT EXECUTE ON FUNCTION public.fn_marcar_extrato_transferencia(uuid,uuid,text) TO authenticated;
