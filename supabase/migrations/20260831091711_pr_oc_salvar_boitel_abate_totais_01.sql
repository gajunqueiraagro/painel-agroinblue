-- PR-OC-VENDA-REALIZADO-02E — a lista branca aceita os tres fatos do papel.
--
-- ⚠ VERSIONAMENTO DE ALGO JA APLICADO. Registro 20260831091711.
--
-- Terceira vez na mesma frente, e o motivo nao muda: esta RPC ENUMERA as colunas, e uma
-- coluna nova sem entrada aqui seria descartada EM SILENCIO — com a auditoria, que grava
-- o payload inteiro, ainda dizendo que gravou. E' a armadilha que ja custou uma PARADA em
-- PR-OC-VENDA-REALIZADO-01A. Coluna e lista branca andam SEMPRE em par.
--
-- ⚠ SEM `COALESCE` nos tres, ao contrario das flags: eles sao `nullable` e sem default na
-- tabela, entao nulo e' valor legitimo — "o abate ainda nao aconteceu". Forcar zero aqui
-- transformaria ausencia em fato, que e' o oposto do que a coluna existe para dizer.
-- `qtd_abatida` usa `::integer` (o idioma do arquiteto nesta migration; as anteriores
-- usam `::int` para `dias` e `morte_quantidade` — os dois sao o mesmo tipo).
--
-- ⚠ RECONSTRUIDA A PARTIR DO REPOSITORIO: corpo de 20260830184100 (md5 08b3015e...,
-- 14345) com as TRES insercoes nos mesmos tres pontos de sempre. Conferido pelo oraculo —
-- md5 b85e0a9a2cb7695e9e8efff5dc555c61, 15110 caracteres, byte a byte igual ao aplicado.
-- As demais 35 colunas nao foram tocadas.
...
BEGIN
  ...
  INSERT INTO public.zoo_operacao_boitel AS b (
    ...
    custo_frete_no_boitel, despesas_abate_no_boitel, custo_notas_envio, notas_envio_no_boitel, data_abate, outros_no_boitel, qtd_abatida, valor_total_abate, acerto_papel,
