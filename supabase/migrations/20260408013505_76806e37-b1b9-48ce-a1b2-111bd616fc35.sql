...
WITH categorias AS (
    ...
    SELECT expanded.*,
           ...
           -- peso contábil acumulado até o mês anterior (para fallback)
    ...
    SELECT pf.*,
           ...
           LAG(pf.fonte) OVER w_cat AS prev_fonte
    FROM pre_final pf
...
SELECT
    ...
    wl.categoria_nome,
    wl.ordem_exibicao,
    -- saldo_inicial: herda do anterior SOMENTE se prev_fonte = 'fechamento'
    CASE
        WHEN wl.mes = 1 THEN wl.cab_ini::integer
        WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_sf
        ELSE wl.cab_inicio_mes::integer
    ...
    -- peso_total_inicial: herda do anterior SOMENTE se prev_fonte = 'fechamento'
    CASE
        WHEN wl.mes = 1 THEN round(wl.peso_ini, 2)
        WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_ptf
        ELSE round(wl.peso_contabil_inicio_mes, 2)
    END AS peso_total_inicial,
    wl.ptf AS peso_total_final,
    -- peso_medio_inicial
    CASE
        WHEN (CASE WHEN wl.mes = 1 THEN wl.cab_ini::integer WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_sf ELSE wl.cab_inicio_mes::integer END) > 0
        THEN round(
            (CASE WHEN wl.mes = 1 THEN round(wl.peso_ini, 2) WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_ptf ELSE round(wl.peso_contabil_inicio_mes, 2) END)
            / (CASE WHEN wl.mes = 1 THEN wl.cab_ini WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_sf ELSE wl.cab_inicio_mes END)::numeric, 2)
        ELSE NULL
    ...
    -- GMD: usa os valores condicionais de saldo_inicial e peso_total_inicial
    CASE
        WHEN (((CASE WHEN wl.mes = 1 THEN wl.cab_ini WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_sf ELSE wl.cab_inicio_mes END)::numeric + wl.sf::numeric) / 2.0) > 0
             AND EXTRACT(day FROM make_date(wl.ano, wl.mes, 1) + '1 mon'::interval - '1 day'::interval) > 0
        THEN round(
            (wl.ptf - (CASE WHEN wl.mes = 1 THEN round(wl.peso_ini, 2) WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_ptf ELSE round(wl.peso_contabil_inicio_mes, 2) END)
            ...
            / (((CASE WHEN wl.mes = 1 THEN wl.cab_ini WHEN wl.prev_fonte = 'fechamento' THEN wl.prev_sf ELSE wl.cab_inicio_mes END)::numeric + wl.sf::numeric) / 2.0)
