...
DROP VIEW IF EXISTS vw_zoot_categoria_mensal;

CREATE VIEW vw_zoot_categoria_mensal AS
WITH categorias AS (
...
), -- Build complete list of category bases: saldo_ini UNION categories that only appear in movements
mov_cat_bases AS (
    SELECT DISTINCT fazenda_id, cliente_id, categoria_id, ano
    FROM mov_all
),
all_cat_bases AS (
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM saldo_ini_cat si
        WHERE si.fazenda_id = mc.fazenda_id AND si.ano = mc.ano AND si.categoria_id = mc.categoria_id
    )
),
expanded AS (
    ...
    SELECT fazenda_id, cliente_id, ano, mes, cenario,
        ...
        peso_ini + COALESCE(sum(peso_entradas_ext - peso_saidas_ext + peso_evol_entrada - peso_evol_saida) OVER w_prev, 0::numeric) AS peso_inicio_contabil,
        cab_ini::numeric + sum(entradas_ext - saidas_ext + evol_cat_entrada - evol_cat_saida) OVER w_curr AS cab_final,
        peso_ini + sum(peso_entradas_ext - peso_saidas_ext + peso_evol_entrada - peso_evol_saida) OVER w_curr AS peso_final_contabil
    FROM expanded
    ...
    CASE
        ...
        THEN round(
            (CASE WHEN r.cenario = 'realizado' AND pr.peso_total_real IS NOT NULL THEN pr.peso_total_real ELSE r.peso_final_contabil END
             - r.peso_inicio_contabil - COALESCE(r.peso_entradas_ext, 0) + COALESCE(r.peso_saidas_ext, 0)
