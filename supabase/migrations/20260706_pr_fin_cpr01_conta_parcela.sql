-- ====================================================================
-- PR-K-bis (26/05/2026) — adiciona p_conta_bancaria_id à RPC
-- fn_reconciliar_parcela_financiamento
-- ====================================================================
-- Adiciona parâmetro opcional p_conta_bancaria_id uuid DEFAULT NULL.
--
-- Regra:
--   - Se p_conta_bancaria_id vier preenchido → vai para conta_bancaria_id
--     do lançamento gravado em financeiro_lancamentos_v2.
--   - Se NULL → fallback para financiamentos.conta_bancaria_id (preserva
--     100% do comportamento atual para callsites que não passam o param).
--
-- Convenção PR-K: parcela é '2-Saídas' → grava em conta_bancaria_id
-- (sem conta_destino_id). RPC já segue isso; aqui apenas permitimos
-- override por chamada (operador escolhe conta na baixa).
--
-- Cobre 2 inserts em financeiro_lancamentos_v2 (criar_principal e
-- criar_juros). O COALESCE entra na montagem dos JSONB v_novo_p e v_novo_j
-- — os INSERTs lêem v_novo->>'conta_bancaria_id', não precisam mudar.
-- ====================================================================

-- ====================================================================
-- PR-FIN-CPR-01 (06/07/2026) — propaga conta bancária no pagamento de
-- parcela de financiamento (motor + backfill controlado).
-- Delta ADITIVO sobre o corpo VIGENTE (== pg_proc, confirmado pelo
-- Architect; base = 20260526_pr_k_bis_02_...):
--   1a) v_conta_desejada := COALESCE(p_conta_bancaria_id, v_contrato.conta_bancaria_id);
--   1b) planejador atualizar_principal passa a comparar conta_bancaria_id;
--   1c) idem atualizar_juros;
--   1d) executor (whitelist por campo) ganha o ramo conta_bancaria_id (::uuid).
-- Guard-rails: nunca gera ação com v_conta_desejada NULL; IS DISTINCT FROM
-- evita ação inútil; respeita p_dry_run. Nada mais muda (criar_*, cancelar_*,
-- C1/C5, alertas, vt, permissões). Sem DROP: 4-param já é a assinatura vigente.
-- PARTE 2 (após o REPLACE): backfill controlado dos 6 lançamentos elegíveis.
-- ====================================================================
...
-- ====================================================================
-- PARTE 2 — Backfill controlado (mesma migration, após o REPLACE).
-- Propaga a conta do financiamento para os lançamentos de parcela dona
-- que nasceram SEM conta (espelho antecipado). Contagem esperada:
-- EXATAMENTE 6 (tabela dos 6 lanc_ids no relatório). ≠6 no dry-run do
-- Architect → REPROVA e para. Não toca valores/datas/status/ano_mes,
-- nem os 26 lançamentos sem conta clara (financiamento sem conta).
-- ====================================================================
UPDATE financeiro_lancamentos_v2 l
 ...
 WHERE (p.lancamento_id = l.id OR p.lancamento_juros_id = l.id)
   AND l.origem_lancamento = 'parcela_financiamento'
...
-- ====================================================================
-- REVERSIBILIDADE (rodapé obrigatório).
-- Estado anterior dos 6: conta_bancaria_id = NULL (todos). Script inverso
-- documentado — reverter pela mesma junção/assinatura:
--   UPDATE financeiro_lancamentos_v2 l
...
--      AND l.cancelado = false
--      AND l.status_transacao = 'realizado'
--      AND l.conta_bancaria_id = f.conta_bancaria_id;
-- (ou, mais estrito, pelos 6 lanc_ids: 65114e11, ab21f9c3, 38894cba,
--  62b1a38c, a6a0799a, 9e0c1e79.) Sem tabela de audit de lançamentos no
-- schema (verificado) → trilha = arquivo + briefing + verificação viva.
-- ====================================================================
