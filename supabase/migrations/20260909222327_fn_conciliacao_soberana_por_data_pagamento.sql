-- ============================================================================
-- 20260909222327 fn_conciliacao_soberana_por_data_pagamento
--
-- A CONCILIACAO RECORTA POR DATA DE PAGAMENTO, NAO POR COMPETENCIA — regra
-- fixada em 09/09/2026. O CTE `lv2` filtrava `l.ano_mes = pp.mes`, que e' a
-- competencia: parcela com competencia de 2024 e lancamento de julho pago em
-- agosto simplesmente sumiam do mes conciliado.
--
-- Derivada da ultima definicao versionada em
-- 20260705_pr_ofx_valido_extrato_valido.sql (md5 do corpo
-- 3024b9a4b5e292ef6b0cd1d2e638c272, 14327) com UMA troca no CTE `lv2`:
--   de:   WHERE l.cliente_id = pp.cli AND l.ano_mes = pp.mes
--   para: WHERE l.cliente_id = pp.cli AND l.data_pagamento BETWEEN pp.d1 AND pp.d2
-- Corpo resultante: md5 e9fae9c9c84b088fd023743c68230384, 14349 — conferido no
-- pg_proc.
--
-- ⚠ A EXPRESSAO APARECE DUAS VEZES NO ARQUIVO, e so' UMA foi trocada. A outra
--    (bloco de diagnostico `resumo.lv2.entradas`, que rele' a tabela por fora do
--    CTE) segue recortando por `ano_mes` — e' o que o banco tem, conferido pelo
--    md5. Fica registrado porque a consequencia e' real: o diagnostico passa a
--    contar por competencia enquanto o CTE conta por pagamento, e os dois vao
--    discordar exatamente nos casos que este PR veio consertar.
-- ============================================================================
