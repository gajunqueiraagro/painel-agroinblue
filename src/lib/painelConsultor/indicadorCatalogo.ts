...
export const CATALOGO_INDICADORES: Record<string, IndicadorMeta> = {
  ...
  // ─── Média do Período ───
  'reb_medio_periodo': { id: 'reb_medio_periodo', nome: 'Rebanho médio período (cab)', aba: 'media_periodo', bloco: 'Desempenho Médio', realizado: FONTE_ZOOT_VIEW_REAL, previsto: FONTE_ZOOT_VIEW_PREVISTO, observacao: 'Média do rebanho de cada mês no período: (Reb. inicial + Reb. final) ÷ 2 por mês, depois média aritmética acumulada de Janeiro até o mês selecionado.' },
  'gmd_medio': { id: 'gmd_medio', nome: 'GMD médio período', aba: 'media_periodo', bloco: 'Desempenho Médio', realizado: FONTE_ZOOT_VIEW_REAL, previsto: FONTE_ZOOT_VIEW_PREVISTO, observacao: 'GMD acumulado: Σ produção biológica (Jan→N) ÷ rebanho médio período (Jan→N) ÷ Σ dias (Jan→N).' },
