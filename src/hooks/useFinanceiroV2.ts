...
export function useFinanceiroV2(pageSize: number = DEFAULT_PAGE_SIZE) {
  ...
  const loadAnosDisponiveis = useCallback(async (): Promise<string[]> => {
    ...
    const { data, error } = await supabase.rpc('get_anos_financeiro_v2', { p_cliente_id: clienteId });
    if (error || !data) {
      ...
      const { data: raw } = await supabase
        ...
        .select('ano_mes')
        .eq('cliente_id', clienteId)
        .neq('status_transacao', 'cancelado');
      const anos = new Set<string>();
      anos.add(String(new Date().getFullYear()));
      (raw || []).forEach((r: any) => {
        if (r.ano_mes && r.ano_mes.length >= 4) anos.add(r.ano_mes.substring(0, 4));
      });
