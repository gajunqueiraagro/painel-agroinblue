...
export function EvolucaoCategoriaTab({ lancamentos, saldosIniciais, initialAno, initialMes, onNavigateToReclass }: Props) {
  ...
  return (
    <div className="p-3 w-full space-y-1 animate-fade-in pb-20">
      ...
      {/* Tabela */}
      <div className="bg-card rounded-lg shadow-sm border overflow-x-auto" style={{ maxWidth: 684 }}>
          <table className="text-[10px]" style={{ tableLayout: 'fixed', width: 684 }}>
            <colgroup>
              <col style={{ width: 75 }} />
              <col style={{ width: 44 }} />
              {COLUNAS_MOV.map(col => (
                <col key={col.tipo} style={{ width: 46 }} />
              ))}
              <col style={{ width: 53 }} />
              <col style={{ width: 52 }} />
            </colgroup>
