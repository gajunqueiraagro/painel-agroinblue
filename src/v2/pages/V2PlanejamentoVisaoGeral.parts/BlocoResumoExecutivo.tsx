...
import { ComposicaoFinanceira, type LinhaModalKey } from './ComposicaoFinanceira';
import { fmtBRL, DeltaBadge } from './resumoExecutivoUI';

// Re-export do tipo para consumidores externos que o importam deste módulo
// (ex.: V2FechamentoPeriodo) — mantém o caminho de import estável.
...
function montarLinhaSaldoFinal(
  ...
  if (modo === 'fechamento') {
    ...
    return {
      ...
      // Fechamento: Saldo Caixa Final é resultado (não custo) → delta direto
      // (real-meta)/meta. Real>Meta = positivo = verde no DeltaBadge (sem
      // inverterSemantica). NÃO usar calcDeltaLocal (semântica de custo).
      deltaAnoCorrente: meta > 0 ? (real - meta) / meta : 0,
    };
  }
  const meta = data.saldoCaixaFinalMeta;
