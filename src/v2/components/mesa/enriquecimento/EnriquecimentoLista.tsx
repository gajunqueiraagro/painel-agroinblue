...
export function EnriquecimentoLista({ rows, selecionadoId, onSelecionar }: EnriquecimentoListaProps) {
  return (
    <div className="rounded-lg border bg-card overflow-hidden">
      <div className="px-2.5 py-1.5 border-b bg-muted/40 flex items-baseline justify-between">
        <span className="text-[10px] uppercase tracking-widest font-bold text-muted-foreground">
          Lançamentos do sistema
        </span>
        <span className="text-[10px] text-muted-foreground/70 tabular-nums">{rows.length}</span>
      </div>
      {rows.length === 0 ? (
        <div className="text-[11px] text-muted-foreground text-center py-8">Nenhuma linha nesta sessão/filtro.</div>
      ) : (
        <div className="space-y-0.5 p-1.5 max-h-[64vh] overflow-y-auto">
          {rows.map((r) => (
            <EnriquecimentoRow
              key={r.id}
              row={r}
