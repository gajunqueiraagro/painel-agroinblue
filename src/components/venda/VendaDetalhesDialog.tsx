      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-1.5 pt-1">
          ...
          <div className="bg-muted/30 rounded p-1.5 grid grid-cols-4 gap-2 text-[11px]">
            ...
            <div><span className="text-muted-foreground">Comprador</span><p className="font-bold truncate">{compradorNome || '-'}</p></div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-4 gap-y-0">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {(['por_kg', 'por_cab', 'por_total'] as const).map(tp => (
                  <button key={tp} type="button"
                    ...
                    className={`h-7 rounded text-[10px] font-bold border-2 transition-all ${tipoPreco === tp ? 'border-primary bg-primary/10' : 'border-border text-muted-foreground'}`}>
              ...
              <div>
                ...
                <Input type="number" value={precoInput} onChange={e => { setPrecoInput(e.target.value); markDirty(); }} placeholder="0,00" className="h-7 text-[10px] w-40" />
              </div>
              {calc.valorBase > 0 && (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input type="number" value={freteLocal} onChange={e => { setFreteLocal(e.target.value); markDirty(); }} placeholder="0,00" className="h-7 text-[10px]" />
                </div>
                <div>
                  <Label className="text-[10px]">Comissão (%)</Label>
                  <Input type="number" value={comissaoPctLocal} onChange={e => { setComissaoPctLocal(e.target.value); markDirty(); }} placeholder="0" className="h-7 text-[10px]" />
                </div>
              </div>
              <div>
                <Label className="text-[10px]">Outros custos extras (R$)</Label>
                <Input type="number" value={outrosCustos} onChange={e => { setOutrosCustos(e.target.value); markDirty(); }} placeholder="0,00" className="h-7 text-[10px] w-40" />
              </div>
              {calc.totalDespesas > 0 && (
                <div className="bg-muted/30 rounded p-1.5 space-y-0.5 text-[10px]">
                  ...
                  {calc.comissaoVal > 0 && (
                    <>
                      {calc.freteVal > 0 && <div className="border-t border-border/30 my-0.5" />}
              ...
              {calc.totalDespesas > 0 && (
                <div className="bg-orange-50 dark:bg-orange-950/30 border border-orange-200 dark:border-orange-800 rounded px-2 py-1 flex justify-between text-[10px] font-bold">
            ...
            <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="0,00" step="0.01" className="h-7 text-[10px]"
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="0,00" className={`h-7 text-[10px] ${funruralPctInput > 0 ? 'bg-muted/40' : ''}`}
                    disabled={funruralPctInput > 0} readOnly={funruralPctInput > 0}
                  />
                </div>
              </div>
              <p className="text-[9px] text-muted-foreground">Informe em % ou R$ — o outro será calculado.</p>
              {calc.totalDeducoes > 0 && (
                <div className="bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded px-2 py-1 flex justify-between text-[10px] font-bold">
              ...
              <div>
                ...
                <Input value={notaFiscal} onChange={e => { setNotaFiscal(e.target.value); markDirty(); }} placeholder="Nº da nota fiscal" className="h-7 text-[10px]" />
              </div>
              <div className="grid grid-cols-2 gap-2">
              ...
              {formaReceb === 'prazo' && (
                <div className="space-y-1">
                  ...
                  {parcelas.map((p, i) => (
                    <div key={i} className="grid grid-cols-2 gap-2 bg-muted/30 rounded p-1">
                      <div>
