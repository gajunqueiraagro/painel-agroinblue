...
function LinhaSum({ rotulo, valor, cor, forte, vazio }: {
  ...
  return (
    <>
      <div className={`${forte ? 'text-[12px] font-semibold text-foreground border-t pt-1 mt-0.5' : `text-[11px] ${cor ?? 'text-muted-foreground'}`}`}>
        {rotulo}
      </div>
      <div className={`text-right tabular-nums ${forte ? 'text-[12px] font-semibold text-foreground border-t pt-1 mt-0.5' : `text-[11px] ${cor ?? 'text-foreground'}`}`}>
...
function ColunaTopo({ rotulo, valor, unidade, linhaAt, subs, evidente, extra }: {
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mt-0.5 text-[15px] font-medium leading-tight tabular-nums truncate">
      ...
      {evidente && <div className="mt-0.5 text-[11px] font-semibold text-foreground tabular-nums">{evidente}</div>}
      {extra}
    </div>
  );
}
  ...
  return (
    <div className="rounded-md border bg-card shadow-sm min-w-0">
      <div className="flex h-7 items-center gap-2 border-b px-2">
        <span className="text-[12px] font-medium text-foreground">Lotes</span>
        ...
        <div className="ml-auto flex gap-0.5 rounded-full border p-0.5">
          {(['projetado', 'realizado'] as CenarioAbate[]).map(c2 => (
            <button key={c2} type="button" onClick={() => onCenarioChange(c2)}
              ...
              className={`rounded-full px-2 py-0 text-[10px] ${c2 === cenario
        ...
        {lotes.length > 0 && (
          <Button type="button" size="sm" variant="outline"
            className="h-[22px] shrink-0 gap-1 px-[9px] text-[10px]"
            onClick={() => setResumoAberto(true)}>
            Resumo dos lotes <ChevronRight className="h-3 w-3" />
          </Button>
        )}
      ...
      {lotes.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-1.5 overflow-auto p-1.5 sm:grid-cols-2 lg:grid-cols-[repeat(4,minmax(0,1fr))]">
          {lotes.map(lote => {
            ...
            return (
              <div key={lote.id} className="flex min-w-0 flex-col gap-0 rounded-md border border-border/60 bg-card px-2 py-1.5">
                <div className="truncate whitespace-nowrap text-[11px] font-medium text-foreground">
                  {lote.categoriaLabel} · {lote.quantidade} cab
                </div>
                <div><Pilula negociado={negociado} /></div>
                {observacaoDoLote && (
                  <div className="truncate whitespace-nowrap text-[10px] text-muted-foreground">
                ...
                <div className="whitespace-nowrap text-[10px] text-muted-foreground">{n2(c.totalArrobas)} @</div>
                <div className="whitespace-nowrap text-[10px] font-semibold text-foreground">
                ...
                <div className="mt-0.5 whitespace-nowrap text-[10px] text-muted-foreground">Líquido do lote</div>
                {negociado ? (<>
                  <div className="whitespace-nowrap text-[11px] font-semibold tabular-nums">{formatMoeda(c.valorLiquido)}</div>
                  <div className="whitespace-nowrap text-[10px] font-semibold tabular-nums">{formatMoeda(c.liqArroba)}/@</div>
