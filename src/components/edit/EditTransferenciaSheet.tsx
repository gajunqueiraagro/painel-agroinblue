 ...
 * - O trigger DB `sync_transferencia_update` propaga alterações da linha
 *   saída para a linha entrada automaticamente.
 * - Este sheet usa o mesmo `onEditar` (= useLancamentos.editarLancamento)
 *   que o fluxo legado já usava — caminho idêntico, sincronização preservada.
 *
...
interface FazendaOption { id: string; nome: string; }
...
interface Props {
  ...
  /** Nome da fazenda atual (origem read-only). */
  nomeFazenda: string;
  /** Lista de fazendas-destino (todas exceto a atual). */
  outrasFazendas: FazendaOption[];
}

export function EditTransferenciaSheet({
  lancamento, open, onOpenChange, onSalvar, onRemover, podeRemover = true,
  canEditMeta = true, p1Oficial = false, temAlteracaoEstrutural,
  nomeFazenda, outrasFazendas,
}: Props) {
  ...
  const handleSalvar = async () => {
    ...
    try {
      ...
      toast.error('Falha ao salvar transferência: ' + (e?.message || 'erro desconhecido'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <EditLancamentoSheet
      ...
      titulo="Editar Transferência (saída)"
      subtitulo="Alterações são propagadas para o lado entrada via trigger do banco."
      banners={
        <>
          {p1Oficial && (
            ...
            <div>
              ...
              {outrasFazendas.length > 0 ? (
                <Select
                  ...
                  onValueChange={v => setForm(f => ({ ...f, fazendaDestino: v }))}
                  disabled={p1Oficial}
                >
          ...
          <div className="flex gap-2 pt-1">
            {podeRemover && onRemover && (
              <Button
                ...
                title={p1Oficial ? 'Mês fechado — exclusão bloqueada' : 'Excluir transferência'}
              >
